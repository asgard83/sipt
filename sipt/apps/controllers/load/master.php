...
class Master extends Controller{
	...
	function list_prioritas($kategori, $prioritas){
		...
		if($substr == "0107"){
			$query = "SELECT SRL_ID, REPLACE(dbo.KATEGORI(GOLONGAN,'1'), '&raquo;', ' - ') AS KATEGORI, PARAMETER_UJI, PUSTAKA, METODE, SYARAT FROM M_PRIORITAS WHERE LEFT(GOLONGAN,2) = '01' $bidang AND PRIORITAS_TAHUN = '".date("Y")."'";
		}else{
			$query = "SELECT SRL_ID, dbo.KATEGORI(GOLONGAN,'1') AS KATEGORI, PARAMETER_UJI, PUSTAKA, METODE, SYARAT FROM M_PRIORITAS WHERE GOLONGAN  = '".$kategori."' $bidang AND PRIORITAS_TAHUN = '".date("Y")."'";
		}
		...
		$this->newtable->search(array(array("REPLACE(dbo.KATEGORI(GOLONGAN,'1'), '&raquo;', ' - ')", 'Berdasarkan Kategori'), array('PARAMETER_UJI', 'Berdasarkan Parameter Uji'), array('PUSTAKA', 'Berdasarkan Pustaka'), array('METODE', 'Berdasarkan Metode'),array('SYARAT', 'Berdasarkan Syarat')));
		
		$this->newtable->columns(array("SRL_ID", "REPLACE(dbo.KATEGORI(GOLONGAN,'1'), '&raquo;', ' - ')", "PARAMETER_UJI", "PUSTAKA", "METODE", "SYARAT"));
