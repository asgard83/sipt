...
CONVERT(VARCHAR(10), A.TANGGAL, 103) AS TANGGAL, B.NAMA_USER, B.USER_ID, B.JABATAN, C.KOTA FROM T_SPK A LEFT JOIN T_USER B ON A.CREATE_BY = B.USER_ID LEFT JOIN M_BBPOM C ON A.BBPOM_ID = C.BBPOM_ID WHERE A.SPK_ID IN (SELECT SPK_ID FROM T_PARAMETER_HASIL_UJI WHERE KODE_SAMPEL = '".$kode."') AND A.CREATE_BY = '".$this->newsession->userdata('SESS_USER_ID')."'")->result_array();
	...
	function printsall($id){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			$page = 1;
			for($x = 0; $x < $idx; $x++){
				$spk_id = $sipt->main->get_uraian("SELECT SPK_ID FROM T_SPK WHERE KODE_SAMPEL = '".$chk[$x]."' AND CREATE_BY = '".$this->newsession->userdata('SESS_USER_ID')."'","SPK_ID");
				if($this->newsession->userdata('SESS_TIPE_BBPOM') == "B" && in_array('B3',$this->newsession->userdata('SESS_SUB_SARANA'))){
					$arrheader = $this->db->query("SELECT dbo.FORMAT_NOMOR('SPK', A.SPK_ID) AS UR_SPK, dbo.FORMAT_NOMOR('SPU',A.SPU_ID) AS SPU, A.SPK_ID, CONVERT(VARCHAR(10), A.TANGGAL, 103) AS TANGGAL FROM T_SPK A LEFT JOIN M_BBPOM C ON A.BBPOM_ID = C.BBPOM_ID WHERE A.SPK_ID = '".$spk_id."' AND A.KODE_SAMPEL = '".$chk[$x]."'")->result_array();
					$mt = "SELECT A.USER_ID, A.NAMA_USER, A.JABATAN, C.KOTA FROM T_USER A LEFT JOIN T_USER_ROLE B ON A.USER_ID = B.USER_ID LEFT JOIN M_BBPOM C ON A.BBPOM_ID = C.BBPOM_ID WHERE B.ROLE = '4' AND B.JENIS_PELAPORAN = '02' AND LEFT(B.SARANA_MEDIA_ID,2) = 'B2' AND A.BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND A.STATUS = 'Aktif' GROUP BY A.USER_ID, A.NAMA_USER, A.JABATAN, C.KOTA ORDER BY 2 ASC";
					$dtmt = $sipt->main->get_result($mt);
					if($dtmt){
						foreach($mt->result_array() as $rowmt){
							$arrheader[0]['USER_ID'] = $rowmt['USER_ID'];
							$arrheader[0]['NAMA_USER'] = $rowmt['NAMA_USER'];
							$arrheader[0]['JABATAN'] = $rowmt['JABATAN'];
							$arrheader[0]['KOTA'] = $rowmt['KOTA'];
						}
					}
				}
				else{
					$arrheader = $this->db->query("SELECT dbo.FORMAT_NOMOR('SPK', A.SPK_ID) AS UR_SPK, dbo.FORMAT_NOMOR('SPU',A.SPU_ID) AS SPU, A.SPK_ID, CONVERT(VARCHAR(10), A.TANGGAL, 103) AS TANGGAL, B.NAMA_USER, B.USER_ID, B.JABATAN, C.KOTA FROM T_SPK A LEFT JOIN T_USER B ON A.CREATE_BY = B.USER_ID LEFT JOIN M_BBPOM C ON A.BBPOM_ID = C.BBPOM_ID WHERE A.SPK_ID IN (SELECT SPK_ID FROM T_PARAMETER_HASIL_UJI WHERE KODE_SAMPEL = '".$chk[$x]."') AND A.CREATE_BY = '".$this->newsession->userdata('SESS_USER_ID')."'")->result_array();
				}
				//print_r($arrheader); die();
				$q = "SELECT B.NAMA_USER FROM T_SPK A LEFT JOIN T_USER B ON A.KASIE = B.USER_ID WHERE A.SPK_ID = '".$spk_id."' AND CREATE_BY = '".$this->newsession->userdata('SESS_USER_ID')."' AND KODE_SAMPEL = '".$chk[$x]."'"; 
				$dt = $sipt->main->get_result($q);
				if($dt){
					foreach($q->result_array() as $rq){
						$arrp[$x][] = $rq;
				...
				if($data){
					...
					$html .= '<tr><td colspan="7" class="'.$grs.'">Tidak ada data untuk no SPK : '.$arrheader[0]['UR_SPK'].'</td></tr>';
				}
				$html .= '</table>';
