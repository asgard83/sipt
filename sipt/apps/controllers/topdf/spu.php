...
class Spu extends Controller{
	...
	function prints($spuid){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			$detil = "SELECT dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL) AS KODE, A.NAMA_SAMPEL, A.NOMOR_REGISTRASI, A.BENTUK_SEDIAAN, A.PABRIK, A.IMPORTIR, dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) AS KOMODITI, dbo.KATEGORI(A.KATEGORI,A.PRIORITAS) AS KATEGORIX, A.NAMA_KATEGORI AS KATEGORI, A.KOMPOSISI, A.KEMASAN, A.KETERANGAN_ED, A.NO_BETS, A.NETTO, A.TEMPAT_SAMPLING, CONVERT(VARCHAR(10), A.TANGGAL_SAMPLING, 103) AS TANGGAL_SAMPLING, A.JUMLAH_SAMPEL, A.JUMLAH_KIMIA, A.JUMLAH_MIKRO, A.SISA, A.EVALUASI_PENANDAAN, A.KETERANGAN_ED, A.CARA_PENYIMPANAN, A.KONDISI_SAMPEL, A.KLASIFIKASI_TAMBAHAN, B.NOMOR_SURAT, dbo.PETUGAS_SAMPLING(A.PERIKSA_SAMPEL) AS PETUGAS_SAMPLING, A.CATATAN, A.UJI_KIMIA, A.UJI_MIKRO FROM T_PERIKSA_SAMPEL B LEFT JOIN T_M_SAMPEL A  ON A.PERIKSA_SAMPEL = B.PERIKSA_SAMPEL WHERE A.SPU_ID = '".$spuid."' AND A.STATUS_SAMPEL NOT IN ('00000') ORDER BY A.KODE_SAMPEL ASC";
			...
			if($res){
				  ...
				  foreach($detil->result_array() as $dt){
					  ...
					  $html .= '<tr>
								...
								<td class="'.$grs.' '.$class.'"><div class="uline">'.$dt['NAMA_SAMPEL'].'</div><div class="uline">&bull; '.$dt['NOMOR_REGISTRASI'].'</div><div class="uline">&bull; '.$dt['BENTUK_SEDIAAN'].'</div><div class="uline">&bull; '.$dt['KOMODITI'].'</div><div class="uline">'.$dt['KATEGORI'].'</div><div class="uline">&bull; '.$dt['PABRIK'].'</div><div class="uline">&bull; '.$dt['IMPORTIR'].'</div><div class="uline">&bull; No. Bets : '.$dt['NO_BETS'].'</div></td>
								<td class="'.$grs.' '.$class.'">'.str_replace(">", " > ", str_replace("<"," < ", $dt['KOMPOSISI'])).'</td>
								<td class="'.$grs.' '.$class.'">'.$dt['KEMASAN'].'</td>
								<td class="'.$grs.' '.$class.'"><div>Tempat Sampling</div><div class="uline">'.$dt['TEMPAT_SAMPLING'].'</div><div>Tanggal Sampling</div><div class="uline">'.$dt['TANGGAL_SAMPLING'].'</div></td>
								...
								$html .= '</ul><div></td>
								<td class="'.$grs.' right">'.preg_replace('/[^(\x20-\x7F)]*/','',$sipt->main->clean_tags($dt['CATATAN'])).'</td>
							  </tr>';
							  $no++;
				  }
				  $html .= '</table>';
			...
			if(!$arr_header[4] == "50201"){
				$html .= '<table width="100%">
					  <tr><td width="33%">Tanggal, </td><td width="33%">Tanggal, </td><td width="33%">Tanggal, </td></tr>
					  <tr><td>Pemohon / Pengirim Sampel</td><td>Penerima Sampel</td><td>Mengetahui </td></tr>
					  <tr><td>(Kepala Bidang / Kasie)</td><td>(Manager Administrasi)</td><td>(Kepala Balai)</td></tr>
					  <tr><td height="75">&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr><tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>
					  <tr><td>NIP.</td><td>NIP.</td><td>NIP. </td></tr></table>';
			}else{
				$kabid = explode("|", $sipt->main->get_uraian("SELECT A.NAMA_USER +'|'+ A.USER_ID +'|'+ CONVERT(VARCHAR(10), B.TANGGAL, 103) AS KABID FROM T_USER A LEFT JOIN T_SPU B ON A.USER_ID = B.TTD_PEMDIK WHERE SPU_ID = '".$spuid."'","KABID"));
				...
				$html .= '<table width="100%">
					  <tr><td width="33%">Tanggal, '.$kabid[2].'</td><td width="33%">Tanggal, '.$ma[2].'</td><td width="33%">Tanggal, '.$mp[2].'</td></tr>
					  <tr><td>Pemohon / Pengirim Sampel</td><td>Penerima Sampel</td><td>Mengetahui </td></tr>
					  <tr><td>(Kepala Bidang / Kasie)</td><td>(Manager Administrasi)</td><td>(Kepala Balai)</td></tr>
					  <tr><td height="75">&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>
					  <tr><td>'.$kabid[0].'</td><td>'.$ma[0].'</td><td>'.$mp[0].'</td></tr>
					  <tr><td>NIP. '.$kabid[1].'</td><td>NIP. '.$ma[1].'</td><td>NIP. '.$mp[1].'</td></tr></table>';
			}
			...
			if($jml > 0){
				...
				for($i=0; $i<$jml; $i++){
					...
					$html .= '<table class="form_tabel" width="100%">
							  ...
							  <tr><td>Nama Sampel</td><td class="kotak">'.$sess[$i]['NAMA_SAMPEL'].'</td><td>Pabrik</td><td class="kotak">'.$sess[$i]['PABRIK'].'</td></tr>    
							  <tr><td>Importir</td><td class="kotak">'.$sess[$i]['IMPORTIR'].'</td><td>Bentuk Sediaan</td><td class="kotak">'.$sess[$i]['BENTUK_SEDIAAN'].'</td></tr>
							  ...
							  <tr><td>Evaluasi Penandaan</td><td class="kotak">'.$sess[$i]['EVALUASI_PENANDAAN'].'</td><td>Cara Penyimpanan</td><td class="kotak">'.preg_replace('/[^(\x20-\x7F)]*/','',$sipt->main->clean_tags($sess[$i]['CARA_PENYIMPANAN'])).'</td></tr>
							  ...
							  <tr><td>Komposisi</td><td class="kotak">'.str_replace(">", " > ", str_replace("<"," < ", $sess[$i]['KOMPOSISI'])).'</td><td>Catatan</td><td class="kotak">'.preg_replace('/[^(\x20-\x7F)]*/','',$sipt->main->clean_tags($sess[$i]['CATATAN'])).'</td></tr>
							  </table>
