  ...
  <div class="adCntnr">
    <div class="acco2">
      ...
      if($stts == "40204" && strlen($sess['KODE_RUJUKAN']) == 0){
        ?>
        <tr id="jmlsampel" style="display:none;">
              <td class="td_left bold">Jumlah Sampel<input type="hidden" value="<?= $sess['SISA']+$sess['SISA_MIKRO']+$sess['SISA_KIMIA']; ?>" id="maxsisasmpl" /> </td>
              <td class="td_right"><input type="text" id="smpltot" name="JUMLAH_SAMPEL" value="<?= $sess['SISA']+$sess['SISA_MIKRO']+$sess['SISA_KIMIA']; ?>" onchange="cekJmlsampel(this);" ></td>
        </tr>
      <tr>
           ...
           <div style="height:5px;">&nbsp;</div>
            ...
            <thead>
              <tr id="jmlnya" style="display:none">                
                <th colspan='5'>Jumlah sampel : <strong style="color:red"><?= $sess['JUMLAH_SAMPEL']; ?></strong><br>
    ...
    ?>
      ...
      if(strlen(trim($sess['KODE_RUJUKAN'])) > 1){
        ?>
        <a href="#" class="button download" id="clhu" url="<?php echo site_url(); ?>/topdf/sampel/rilis/cp-lcp-lhu/<?php echo $sess['KODE_RUJUKAN']; ?>" onclick="blank_($(this)); return false;"><span><span class="icon"></span>&nbsp; Preview CP & LHU Balai Asal &nbsp;</span></a>
        <?php
      }
    }
    ?>
      ...
      $("#draft-rujukan").empty();
        ...
        if($(this).is(':checked')){
          ...
          var paramuji = $("#"+tr+" td:nth-child(3)").text(); 
            ...
            clone += '<td><input type="text" name="RUJUKAN[JUMLAH_SAMPEL][]" class="text jumsampel" rel="required" onchange="cekjmldetil(this);" /></td>';
            clone += '</tr>';
...
function cekJmlsampel(e){
  var maxsisa = $("#maxsisasmpl").val();
  var val = e.value;
  if(val > maxsisa){
    alert('Jumlah Sampel tidak boleh melebihi sisa sampel.');    
    e.value = maxsisa;

  }
} 

function cekjmldetil(e){
  var totsmpl = $("#smpltot").val();
  var a = parseInt(0);
  $('.jumsampel').each(function(){
      if(this.value != "")
        a += parseFloat(this.value);
  })
    
  if(a > totsmpl){
    alert('Sampel tidak boleh melebihi jumlah sampel.');    
    e.value = 0;
  }
}
