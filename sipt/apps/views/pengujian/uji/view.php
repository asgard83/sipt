    ...
    <div class="collapse">
      <div class="accCntnt">
        ...
        <table class="form_tabel" width="100%">
          ...
          <tr>
            <td class="td_left">Sisa sampel <?php echo in_array('B1',$this->newsession->userdata('SESS_SUB_SARANA')) || in_array('B2',$this->newsession->userdata('SESS_SUB_SARANA')) ? 'kimia' : 'mikro'; ?></td>
            <td class="td_right"><?php echo in_array('B1',$this->newsession->userdata('SESS_SUB_SARANA')) || in_array('B2',$this->newsession->userdata('SESS_SUB_SARANA')) ? $sampel[0]['SISA_KIMIA'] : $sampel[0]['SISA_MIKRO']; ?></td>
          </tr>
        </table>
        <?php
                 $jparams = count($sess);
        ...
        <div>
          ...
          <table class="form_tabel">
            ...
            <tr>
              ...
              <td class="td_right"><?php echo $sess[$i]['HASIL_PARAMETER']; ?></td>
              <td></td>
              <td class="td_left">&nbsp;</td>
              <td class="td_right">&nbsp;</td>
            </tr>
            <tr>
              ...
              <td class="td_right"><?php
                ...
                <?php
									if($sess[$i]['PENGUJI'] == $this->newsession->userdata('SESS_USER_ID')){ 
								  	?>
                <p>Jika file Laporan Catatan Pengujian (LCP) tidak bisa didownload atau gagal upload</p>
                <p><b>silahkan klik <a href="javascript:void(0);" class="reupload-lcp" url="<?php echo site_url(); ?>/get/pengujian/reupload_lcp/<?php echo $kode_sampel; ?>/<?php echo $sess[$i]['UJI_ID']; ?>">disini</a> untuk melakukan upload ulang atau klik <a href="javascript:void(0);" class="delete-lcp" url="<?php echo site_url(); ?>/get/pengujian/delete_lcp/<?php echo $sess[$i]['UJI_ID']; ?>">disini</a> untuk membatalkan LCP</b></p>
                <?php
									}
							  }else{
								  if($sess[$i]['PENGUJI'] == $this->newsession->userdata('SESS_USER_ID')){ 
								  ?>
                <p>Jika file Laporan Catatan Pengujian (LCP) tidak bisa didownload atau gagal upload</p>
                <p><b>silahkan klik <a href="javascript:void(0);" class="reupload-lcp" url="<?php echo site_url(); ?>/get/pengujian/reupload_lcp/<?php echo $kode_sampel; ?>/<?php echo $sess[$i]['UJI_ID']; ?>">disini</a> untuk melakukan upload ulang</b></p>
                <?php
								  }else{
									  ?>
                <p>Tidak Melampirkan Laporan Catatan Pengujian</p>
                <?php
								  }
							  }
							  ?></td>
            ...
            <tr>
              <td class="td_left" colspan="5"><div style="background:#FBE3E4; border:1px solid #ccc; padding:5px;">
                  <p><b>Keterangan : Hasil Pengujian TMS (Tidak Memenuhi Syarat) wajib melampirkan LCP</b></p>
                </div></td>
            </tr>
          </table>
        </div>
...
<div id="reupload-lcpx"></div>
<script type="text/javascript">
$(document).ready(function(){
	$(".reupload-lcp").click(function(){
		$.get($(this).attr("url"), function(data){
			$("#reupload-lcpx").html(data); 
			$("#reupload-lcpx").dialog({ 
	...
	$(".delete-lcp").click(function(){
		var url = $(this).attr("url");
		jConfirm('Anda yakin akan menghapus data LCPterpilih ? \n Harap diperhatikan, bahwa data LCP yang telah dihapus tidak bisa dikembalkan lagi.', 'SIPT Versi 1.0', function(r){
			if(r){
