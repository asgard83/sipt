          ...
          <div class="accCntnt">
            ...
            <table class="form_tabel">
              <tr>
                ...
                <td class="td_right"><?php echo form_dropdown('SAMPEL[ASAL_SAMPEL]',$asal,$sess['ASAL_SAMPEL'],'class="stext" title="Asal Sampel" id="asal_sampling" rel="required" url="'. site_url() .'/autocompletes/autocomplete/operator/'.$this->newsession->userdata('SESS_BBPOM_ID').'/"'); ?></td>
              </tr>
              <tr>
                <td class="td_left">Tanggal Surat Tugas </td>
                <td class="td_right"><input type="text" class="sdate" title="Tanggal Surat Tugas" name="SURAT[TANGGAL_SURAT]" rel="required" readonly id="tanggal_surat" value="<?php echo $sess['TANGGAL_SURAT']; ?>" /></td>
                <td></td>
                <td></td>
                <td><input type="hidden" name = "SAMPEL[PRIORITAS]" id="cbprioritas" value = "1"/></td>
              </tr>
              <tr>
                <td class="td_left">&nbsp;</td>
              ...
              <tr>
                ...
                <td class="td_right"><div style="padding-bottom:5px;"> <?php echo form_dropdown('SAMPEL[TUJUAN_SAMPLING]',$tujuan,$sess['TUJUAN_SAMPLING'],'class="stext" title="Tujuan Sampling" rel="required" id="tujuan_sampling" url="'.site_url().'/autocompletes/autocomplete/get_tujuan_sampling/"'); ?> </div>
                  <div <?= (array_key_exists('SUB_TUJUAN', $sess)) && ($sess['SUB_TUJUAN'] != "") ? '' : 'style="display:none;"'; ?> id="div_tujuan"> <?php echo form_dropdown('SAMPEL[SUB_TUJUAN]',$sub_tujuan,$sess['SUB_TUJUAN'],'class="stext" title="Sub Tujuan Sampling" id="sub_tujuan"'); ?> </div></td>
              </tr>
              <tr>
                ...
                <td class="td_right"><input type="text" class="sdatesampling" title="Tanggal Sampling" name="SAMPEL[TANGGAL_SAMPLING]" rel="required" value="<?php echo $sess['TANGGAL_SAMPLING']; ?>" id="tanggal_sampling" onchange="chk_tgl('#tanggal_surat', '#tanggal_sampling', 'Mohon diperiksa kembali tanggal sampling tidak boleh lebih kecil dari tanggal surat tugas'); return false;" readonly /></td>
              </tr>
              <tr>
                <td class="td_left">Petugas Sampling</td>
                <td class="td_right"><input type="text" class="stext operator" id="operator" url="<?php echo site_url(); ?>/autocompletes/autocomplete/operator/<?php echo $this->newsession->userdata('SESS_BBPOM_ID') ?>/" title="Ketikan nama petugas, lalu tekan enter untuk menambahkan nama petugas." />
            ...
            <div class="biaya-pihak-ke-3" style="display:none;">
              <table class="form_tabel">
                <tr>
                  ...
                  <td class="td_right"><input type="text" class="stext w100" title="Biaya" name="SURAT[BIAYA]" value="<?php echo $sess['BIAYA']; ?>" /></td>
                </tr>
                <tr>
        ...
        <div class="collapse">
          <div class="accCntnt">
            ...
            <table class="form_tabel">
              ...
              <tr id="info-kategori" style="display:none;">
                <td colspan="5" class="td_left"><div style="background:#FBE3E4; border:1px solid #ccc; padding:5px;"> <i>* Untuk kategori F dan I jika ingin menambahkan data kategori prioritas sampling silahkan klik <a href="javascript:void(0);" class="ahrefkategori">disini</a></i> </div></td>
              </tr>
              <tr>
                <td class="td_left">Kategori Tambahan</td>
                <td class="td_right" colspan="5"><?php echo form_dropdown('SAMPEL[KLASIFIKASI_TAMBAHAN]',$klasifikasi_tambahan,$sess['KLASIFIKASI_TAMBAHAN'],'class="stext" title="Komoditi" id="kk_tambahan" style="width:500px;"'); ?></td>
              </tr>
              <tr>
...
<div id="panel-prioritas"></div>
<script type="text/javascript">

$(document).ready(function(){
	$(".surveilance-d2").css("display","none");
	...
	$("#asal_sampling").change(function(){
		...
		var asalsampling = $(this).val();

		var urlasli = $("input.operator").attr("url");

		if(asalsampling!="13"){

			$("input.operator").attr("url", $(this).attr("url"));

		}else{

			$("input.operator").attr("url", ($(this).attr("url") + $(this).val()));
  ...
  var tahunskrng = tgl.getFullYear();  
	...
	$('input.sdatesampling').datepicker({ dateFormat: 'dd/mm/yy',regional: 'id', minDate: new Date(tahunskrng, 01 - 1, 1), maxDate: new Date()});  
	//$('input.sdate2').datepicker({ dateFormat: 'mm/yy',regional: 'id'});	
	...
	$('select.komoditi').change(function(){
		...
		var prioritasx = $("#cbprioritas").val();

		var tujuan = $("#tujuan_sampling option:selected").val();

		if(tujuan=="05") prioritasx = "0";
		...
		if(ke == 1){
			...
			if($this.val() == "0110" || $this.val() == "0111" && prioritasx == "1"){

				$("tr#info-kategori").show();

				$("a.ahrefkategori").attr("url", "<?php echo site_url(); ?>/get/pengujian/prioritas/" + $this.val());

			}else{

				$("tr#info-kategori").hide();

				$("a.ahrefkategori").removeAttr("url");

			}

			if($this.val() == "0106"){
				$(".surveilance-d2").css("display","");
				if($(".surveilance-d2").length > 0){
					$("#spl-surveilance-d2").attr("rel","required");	
				}
			}else{
				$(".surveilance-d2").css("display","none");
				$("#spl-surveilance-d2").removeAttr("rel");
			}

			var chk = kunci.substring(0, 2);

			if(chk == '01'){

				$.get('<?php echo site_url().'/autocompletes/autocomplete/get_kk_tambahan/'; ?>' + $(this).val(), function(hasil){
		...
		else if(ke == 3 && $substr == "0107" && prioritasx == "1"){
		...
		else if((ke == 3 || ke == 4 || ke == 5) && $this.val().substring(0,6) == '011001'  && prioritasx == "1"){
			var $id = $this.attr("id");
			var $jenis = $("#" + $id + " option:selected").text();
			if(ke == 5) $("#nama_sampel").attr("url","<?= site_url(); ?>/autocompletes/autocomplete/get_rokok/" + $jenis);			
			else  $("#nama_sampel").attr("url","<?= site_url(); ?>/autocompletes/autocomplete/get_rokok/");
			$("#nama_sampel").autocomplete($("#nama_sampel").attr('url'), {width: 244, selectFirst: false});
			$("#nama_sampel").result(function(event, data, formatted){
				if(data){//console.log(data);
					$("#nama_sampel").val(data[1]);
					$(this).val(data[1]);
					$("#pabrik").val(data[2]);
					$("#komposisi").html(data[3]);
					$("#nie").val('');
				}
			});
		}
		...
		if(kunci != "01" && kunci != "02" && kunci != "05" ){
			...
			if(komoditi == "10" || komoditi == "11" || komoditi == "12"){
				...
				$.get($(this).attr("url") + komoditi + '/' + kunci, function(hasil){
					...
					if(hasil.length > 1){console.log(hasil);
	...
	$("#sub_tujuan").change(function(e){
		...
		if($komoditi == "10" || $komoditi == "12"){

			$.get('<?php echo site_url().'/autocompletes/autocomplete/get_kategori/'; ?>' + $komoditi + '/' + $prioritasx + '/' + $tujuan + '/' + $this.val(), function(hasil){

				var hasil = hasil.replace(' ', ''); 

				var jum = hasil.length;

				if(jum == 0 || $this.val() == 00){
	...
	$(".ahrefkategori").click(function(){

		$.get($(this).attr("url"), function(data){

			$("#panel-prioritas").html(data); 

			$("#panel-prioritas").dialog({

				title: 'Master Data Prioritas Sampling',
