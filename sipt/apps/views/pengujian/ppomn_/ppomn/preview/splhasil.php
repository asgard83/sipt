    	...
    	<div class="acco2">
        	<div class="expand"><a title="expand/collapse" href="#" style="display: block;">DATA HASIL SAMPEL </a></div>
				...
				<form name="fsplhasil" id="fsplhasil" method="post" action="<?php echo $act; ?>" autocomplete="off">
					...
					<?php
						...
						if($jparameter > 0){
							for($x = 0; $x < $jparameter; $x++){
								?>
								<tr><td><?php echo $parameter[$x]['JENIS_UJI']; ?></td><td><?php echo $parameter[$x]['PARAMETER_UJI']; ?></td><td><?php echo $parameter[$x]['HASIL']; ?></td><td><?php echo $parameter[$x]['SYARAT']; ?></td><td><?php echo $parameter[$x]['METODE']; ?></td><td><?php echo $parameter[$x]['PUSTAKA']; ?></td><td><a href="<?php echo base_url().'files/LCP/'.$sess['KODE_SAMPEL'].'/'.$parameter[$x]['LCP']; ?>" target="_blank">LCP</a></td></tr>
								<?php
							}
						}
					?>
					...
					?>
					<tr>
						<td class="td_left bold">Hasil PPOMN</td><td class="td_right"><?php echo form_dropdown('HASIL_PPOMN',$hasil,'','class="stext" title="Hasil uji absah PPOMN" id="hasil" rel="required"'); ?></td>
					</tr>
					<?php
					}
					?>
