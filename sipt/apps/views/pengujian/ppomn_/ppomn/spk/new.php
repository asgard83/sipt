        ...
        <form name="fspk" id="fspk" method="post" action="<?php echo $act; ?>" autocomplete="off">
          <table class="form_tabel">
            ...
            <tr class="row-petugas" ke="0" id="petugas">
              ...
              <td class="td_right"><input type="text" class="stext operator"  url="<?php echo site_url(); ?>/autocompletes/autocomplete/get_petugas_pengujian/3/<?php echo $spuid; ?>" title="Ketikan Nama Penyelia, kemudian tekan enter" />
                <input type="hidden" id="ippetugas-0" name="USER_ID[]" />
                &nbsp;<a href="#" class="addpetugas"><img src="<?php echo base_url(); ?>images/add.png" align="top" style="border:none" /></a>
                <div style="padding-top:5px; font-size:9px; font-weight:bold;">Klik tanda plus disamping untuk menambah penyelia baru</div></td>
            </tr>
          </table>
          <?php
		  if(!in_array('B5',$this->newsession->userdata('SESS_SUB_SARANA'))){
			  if($chkmikro > 0){
			  ?>
          <input type="hidden" name="chkmikro" value="<?php echo $chkmikro; ?>" />
          <div style="height:5px;">&nbsp;</div>
          <div style="background:#ffffa4; border:1px solid #ccc; padding:5px;">
            <h2 class="small">Form Permohonan Uji Ke Bidang Mikro</h2>
            <table class="form_tabel">
              <tr>
                <td class="td_left">Nomor Surat Permintaan Uji</td>
                <td class="td_right bold"><?php echo $dtspu[0]['UR_FORMAT']; ?></td>
              ...
              <tr>
                <td class="td_left">Ditujukan Kepada Bidang Mikrobiologi</td>
                <td class="td_right bold"><input type="text" class="stext kabidmikro" title="Masukan nama kabid mikrobiologi, kemudian enter untuk memilih" url="<?php echo site_url(); ?>/autocompletes/autocomplete/get_dispo" />
                <input type="hidden" id="kabid" name="SP_PETUGAS" /></td>
              </tr>
            </table>
          </div>
          <?php
          ...
          <div>
            ...
            <table class="tabelajax">
              ...
              <tr>
                <td><?php echo $no; ?>.</td>
                <td><div><?php echo $arr[$i]['NAMA_SAMPEL']; ?></div>
                  <div><?php echo $sess['KODE']; ?></div>
                ...
                <td><input type="text" style="height:20px;" class="stext paramuji" title="Ketik nama parameter uji, kemudian tekan enter untuk menambahkan parameter uji" id="<?php echo $arr[$i]['KATEGORI']; ?>" url="<?php echo site_url(); ?>/autocompletes/autocomplete/get_jenis_pengujian/<?php echo $arr[$i]['KATEGORI']; ?>"  ke="<?php echo $i; ?>" spl="<?php echo $arr[$i]['KODE_SAMPEL']; ?>"/>
                  <div>
                    <ul style="margin:0px; padding:0px; list-style:none;" id="param_ke_<?php echo $i; ?>" >
                    </ul>
                  </div></td>
                <?php
				   $no++;
...
$(document).ready(function(){
	...
	$(".addpetugas").live("click", function(){
		...
		var str = '<tr id="row-petugas" ke="'+(jml+1)+'" id="petugas'+(jml+1)+'"><td class="td_left">&nbsp;</td><td class="td_right"><input type="text" class="stext operator" url="<?php echo site_url(); ?>/autocompletes/autocomplete/get_petugas_pengujian/3/<?php echo $spuid; ?>" title="Ketikan Nama Penyelia, kemudian tekan enter" /><input type="hidden" id="ippetugas-'+(jml+1)+'" name="USER_ID[]" />&nbsp;<a href="#" class="removepetugas"><img src="<?php echo base_url(); ?>images/cancel.png" align="top" style="border:none" /></a></td></tr>';
	...
	$("input.kabidmikro").autocomplete($("input.kabidmikro").attr('url'), {width: 244, selectFirst: false});
	$("input.kabidmikro").result(function(event, data, formatted){
		if(data){
			$(this).val(data[2]);
			$("#kabid").val(data[1]);
		}
