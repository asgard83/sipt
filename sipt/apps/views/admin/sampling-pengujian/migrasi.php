...
&nbsp;Start Migrasi, Interval&nbsp;&nbsp;
<input type="text" class="sdate" name="intreset" id="intreset" value="2" title="Interval timer">
<div style="height:2px;">&nbsp;</div>
<div id="timer_reset" style="padding-top:5px; padding-bottom:5px;"></div>
<div style="height:2px;">&nbsp;</div>
<ul id="isimigrasi" style="list-style:none; padding:0px; margin-left:5px;">
</ul>
<script>
	...
	function set_kode(){
		...
		if(idx<arrid.length){
			var url = '<?= site_url();?>/utility/tools/get_migrasi/' + arrid[idx];
			var percent = 0;
			$.get(url, function(data){
				arrdata = data.split('#'); 
				if(arrdata[0].trim()=='OK'){
					percent = parseFloat((idx) / arrid.length) * 100;
					$("ul#isimigrasi").html(arrdata[1]);
