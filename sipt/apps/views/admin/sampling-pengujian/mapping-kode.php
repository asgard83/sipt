...
&nbsp;Start&nbsp;&nbsp;
<input type="text" class="sdate" name="intreset" id="intreset" value="3" title="Interval timer">
...
<script>
	...
	function fstart(){
		if($('#chk_start').attr("checked")){
			...
			var beginLoad = setInterval(function(){ load(); }, 10000);
		}else{
	...
	function set_kode(){
		...
		if(idx<arrid.length){
			var url = '<?= site_url();?>/utility/tools/set_mapping_kode/' + arrid[idx];
			var percent = 0;
