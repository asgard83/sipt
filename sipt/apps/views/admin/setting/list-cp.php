...
<table class="listtemuan" width="100%">
  <thead>
    <tr>
      ...
      <th>Reset</th>
      <th width="80"><input type="checkbox" id="chk-all" data-balai = "<?php echo $balai; ?>" data-bidang = "N" data-komoditi = "N" data-url = "<?php echo site_url(); ?>/utility/setting/set_cp/multi/" />&nbsp;Auto Reset</th>
	...
	if($jml > 0){
		for($x = 0; $x < $jml; $x++){
            ...
            <tr <?php echo $sess[$x]['BIDANG'] == "K" ? 'style="background-color:#EBF1DE;"' : 'style="background-color:#fcfcfc;"'; ?>>
            	...
            	<td align="center"><input type="checkbox" <?php echo $sess[$x]['AUTO_RESET'] == "0" ? '' : 'checked=\"checked"'; ?>  value="<?php echo $sess[$x]['AUTO_RESET'] == "0" ? '0' : '1'; ?>" class="chk-reset" data-balai = "<?php echo $sess[$x]['BBPOM_ID']; ?>" data-anggaran = "<?php echo $sess[$x]['ANGGARAN']; ?>" data-komoditi = "<?php echo $sess[$x]['KOMODITI']; ?>" data-bidang = "<?php echo $sess[$x]['BIDANG']; ?>" data-url = "<?php echo site_url(); ?>/utility/setting/set_cp/single/" /></td>
		...
		$("#chk-all").click(function(){
			...
			if($this.is(":checked")){
				jConfirm('Proses data terpilih sekarang ?', 'SIPT Versi 1.0', function(ojan){
					if(ojan){
						$.get($this.attr("data-url") + $this.attr("data-balai") + '/' + $this.attr("data-komoditi") + '/' + $this.val(), function($hasil){
							if($hasil){
								var $arr = $hasil.split("#");
								if($arr[1] == "YES"){
