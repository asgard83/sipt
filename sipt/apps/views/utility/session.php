    ...
    <table class="tabelajax">
		...
		<?php
			...
			for($i=0; $i<$jml;$i++){
				...
				?>
                <tr>
                    <td><?php echo $no;?></td><td><?php echo str_replace('"','',$c['SESS_USER_ID']);?><div><?php echo str_replace('"','',$c['SESS_NAMA_USER']);?></div></td><td><?php echo str_replace('"','',$c['SESS_MBBPOM']);?><div>Jabatan : <?php echo str_replace('"','',$c['SESS_JABATAN']);?></div></td><td>Login Terakhir : <div><?php echo str_replace('"','',$c['SESS_LOGIN']);?></div>Logout Terkahir : <div><?php echo str_replace('"','',$c['SESS_LOGOUT']);?></div></td><td>IP Address : <div><?php echo str_replace('"','',$c['SESS_IP']);?></div>Browser : <div><?php echo str_replace('"','',$c['SESS_AGENTS']);?></div></td>
                </tr>
                <?php
                $no++;
			}
