...
<form name="fedit-penyelia" id="fedit-perbaikan-balai" method="post" action="<?php echo $act; ?>" autocomplete="off">
  ...
  <table class="form_tabel">
    ...
    <tr>
      ...
      <td class="td_right"><?php echo $sess['TANGGAL_TINDAKAN_BALAI']; ?></td>
    </tr>
    <tr>
      <td class="td_left">Saran Tindak Lanjut</td>
      <td class="td_right"><?php echo $sess['DETAIL_TINDAKAN_BALAI']; ?></td>
    </tr>
    <?php
    ...
    <tr>
	  ...
	  if($sess['FILE_TL_BALAI'] != "" && trim($sess['FILE_TL_BALAI']) != ""){
		  ?>
        <a href="<?php echo base_url(); ?>files/<?php echo $sess['SARANA_ID']; ?>/<?php echo $sess['FILE_TL_BALAI']; ?>" target="_blank">File Lampiran</a>
        <?php
	  }else{
		  ?>
        -
  ...
  ?>
    ...
    <tr>
      ...
      <td class="td_right"><span class="upload_PERBAIKAN">
        <input type="file" class="stext upload" jenis="PERBAIKAN" allowed="rar-zip" url="<?php echo site_url(); ?>/utility/uploads/get_upload/<?php echo $sess['SARANA_ID'];?>" id="fileToUpload_PERBAIKAN" name="userfile" onchange="do_upload($(this)); return false;"/>
        &nbsp;Tipe File : *.rar, *.zip</span><span class="file_PERBAIKAN"></span></td>
    </tr>
  </table>
  <div style="height:10px;">&nbsp;</div>
  <div style="padding-left:5px;"><a href="#" class="button save" onclick="save_edit('#fedit-perbaikan-balai'); return false;"><span><span class="icon"></span>&nbsp; Proses &nbsp;</span></a></div>
  <div style="height:10px;">&nbsp;</div>
</form>
<script>
