...
<div class="content">
	...
	<form action="<?php echo $act; ?>" autocomplete="off" method="post" id="rhpk">
		...
		<tr><td class="td_left">Periode Tahun</td><td class="td_right"><input type="text" class="sdate" name="AWAL" id="tglAwalPengawasan<?php echo $i; ?>" title="Tanggal Pengawasan Iklan" onchange="comp('A')" rel='required'/>&nbsp; sampai dengan&nbsp;
			<input type="text" class="sdate" name="AKHIR" id="tglAkhirPengawasan<?php echo $i; ?>" title="Tanggal Pengawasan Iklan" onchange="comp('B')" rel='required'/></td></tr>
		...
		<?php } ?>
		<tr><td class="td_left">Hasil</td><td class="td_right"><select name="HASIL" class="stext" title="Hasil Pengawasan"><option value=""></option><option value="MK">MK</option><option value="TMK">TMK</option></select></td></tr>
	    </table>
