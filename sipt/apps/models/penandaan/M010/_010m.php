...
class _010M extends Model {
	...
	if ($this->newsession->userdata('LOGGED_IN') == TRUE) {
	    ...
	    $arrdata ['cb_tindakan'] = $this->config->item('tl_pusat_penandaan_ot');
	    $arrdata ['cb_tindakan_balai'] = $this->config->item('tl_balai_penandaan_ot');
	    $arrdata['editTL'] = 'YES';
	...
	$arr_penandaan_OT_BL = join("#", $arrChkBLVal);
	...
	if ($this->db->affected_rows() > 0) {
	    $penandaanOT = array('PENANDAAN_ID' => $penandaan_id, 'PENILAIANBL' => $arr_penandaan_OT_BL, 'PENILAIANKP' => $arr_penandaan_OT_KP, 'URAIANBL' => $arr_penandaan_URN_BL, 'URAIANKP' => $arr_penandaan_URN_KP, 'LAMPIRAN' => $lampiran, 'TL_BALAI' => $arr_penandaan['TL_BALAI'], 'PEMUSNAHAN' => $pemusnahan, 'SISTEM' => $hasilSistem['HASIL'], 'PUSAT' => $hasilPusat);
	}
	$this->db->insert('T_PENANDAAN_OT', $penandaanOT);
	if ($this->db->affected_rows() > 0) {
	    $case = 'YESPENANDAANOT';
	} else {
	    $case = 'NO';
	}
	if ($case == 'YESPENANDAANOT') {
	...
	foreach ($penandaanId as $penandaan_id) {
		...
		$saranaUPD = array('TANGGAL_MULAI' => $arr_penandaan['TANGGALAWAL'], 'TANGGAL_AKHIR' => $arr_penandaan['TANGGALAKHIR'], 'SARANA_ID' => $arr_penandaan['SARANAID'], 'KOMODITI' => '010');
	    }
	    ...
	    if ((!in_array($this->newsession->userdata('SESS_BBPOM_ID'), $this->config->item('cfg_unit')))) {
		$penandaanOT = array('PENILAIANBL' => $arr_penandaan_OT_BL, 'PENILAIANKP' => $arr_penandaan_OT_KP, 'URAIANBL' => $arr_penandaan_URN_BL, 'URAIANKP' => $arr_penandaan_URN_KP, 'LAMPIRAN' => $lampiran, 'TL_BALAI' => $arr_penandaan['TL_BALAI'], 'PEMUSNAHAN' => $pemusnahan, 'SISTEM' => $hasilSistem['HASIL'], 'PUSAT' => $hasilPusat);
	    } else {
		if (!empty($arr_penandaan_OT_BL))
		    $penandaanOT = array('PENILAIANBL' => $arr_penandaan_OT_BL, 'PENILAIANKP' => $arr_penandaan_OT_KP, 'URAIANBL' => $arr_penandaan_URN_BL, 'URAIANKP' => $arr_penandaan_URN_KP, 'LAMPIRAN' => $lampiran, 'SISTEM' => $hasilSistem['HASIL'], 'PUSAT' => $hasilPusat);
		else
		    $penandaanOT = array('PUSAT' => $hasilPusat);
	    }
	    $this->db->where(array("PENANDAAN_ID" => $penandaan_id));
	    $this->db->update('T_PENANDAAN_OT', $penandaanOT);
	    $case = 'YESPENANDAANOT';
	}
	if ($case == 'YESPENANDAANOT') {
	...
	$sipt = & get_instance();
	$this->load->model("main", "main", true);
	$query = "SELECT TPOT.URAIANBL, TPOT.URAIANKP, TPOT.PENILAIANBL, TPOT.PENILAIANKP, TPOT.PUSAT, TPOT.SISTEM, TP.PENANDAAN_ID, CONVERT(VARCHAR(10), TP.TANGGAL_MULAI, 120) AS TANGGAL_MULAI, CONVERT(VARCHAR(10), TP.TANGGAL_AKHIR, 120) AS TANGGAL_AKHIR, TP.SURAT_ID, TP.STATUS FROM T_PENANDAAN TP RIGHT JOIN T_PENANDAAN_OT TPOT ON TPOT.PENANDAAN_ID = TP.PENANDAAN_ID WHERE TPOT.PENANDAAN_ID = '" . $idPengawasan . "'";
	$data = $sipt->main->get_result($query);
	if ($data) {
	    foreach ($query->result_array() as $row) {
		$arrdata = array('sess' => $row, 'judul' => 'Obat Tradisional');
	    }
	    if ($row['STATUS'] == '20302' || $row['STATUS'] == '20312') {
		$arrdata['act'] = site_url() . '/penandaan/penandaanController/prosesEdit/edit/' . $row['PENANDAAN_ID'] . '/' . $row['STATUS'] . '/010/' . $jenisPelaporan;
		$arrdata['tombol'] = 'Lihat Data Perbaikan Pengawasan';
	    } else {
		$arrdata['act'] = site_url() . '/penandaan/penandaanController/prosesPreview/preview/' . $row['PENANDAAN_ID'] . '/' . $row['STATUS'] . '/010/' . $jenisPelaporan;
	...
	$query = "SELECT TSTP.SURAT_ID, TPOT.PENILAIANBL, TPOT.PENILAIANKP, TPOT.URAIANBL, TPOT.URAIANKP, CONVERT(VARCHAR(MAX), TPOT.LAMPIRAN) AS LAMPIRAN, TPOT.PUSAT, TPOT.TL_BALAI, TPOT.SISTEM, TPOT.PEMUSNAHAN, TP.PENANDAAN_ID, CONVERT(VARCHAR(10), " . $tglQuery . ", TP.STATUS, TP.KOMODITI, TP.SARANA_ID, TP.BBPOM_ID, TPP.NAMA_PRODUK, TPP.BENTUK_SEDIAAN, TPP.BESAR_KEMASAN, TPP.KOMPOSISI, TPP.PRODUSEN, TPP.ALAMAT_PRODUSEN, TPP.PENDAFTAR, TPP.ALAMAT_PENDAFTAR, TPP.NOMOR_IZIN_EDAR, TPP.GOLONGAN_PRODUK, TPP.NOMOR_PRODUK, MS.NAMA_SARANA, MS.ALAMAT_1, (SELECT MP1.NAMA_PROPINSI FROM M_PROPINSI MP1 WHERE MP1.PROPINSI_ID = MS.PROPINSI) AS PROPINSI, (SELECT MP2.NAMA_PROPINSI FROM M_PROPINSI MP2 WHERE MP2.PROPINSI_ID = MS.KOTA) AS KOTA FROM T_PENANDAAN TP LEFT JOIN T_PENANDAAN_OT TPOT ON TPOT.PENANDAAN_ID = TP.PENANDAAN_ID LEFT JOIN T_PENANDAAN_PRODUK TPP ON TPP.PENANDAAN_ID = TP.PENANDAAN_ID LEFT JOIN T_SURAT_TUGAS_PENANDAAN TSTP ON TSTP.SURAT_ID = TP.SURAT_ID LEFT JOIN M_SARANA MS ON MS.SARANA_ID = TP.SARANA_ID WHERE TP.PENANDAAN_ID = '" . $penandaanId . "'";
	$data = $sipt->main->get_result($query);
	if ($data) {
	    foreach ($query->result_array() as $row) {
		$arrdata = array('sess' => $row, 'judul' => 'Obat Tradisional');
	...
	if ($status == '20502' || $status == '20512' || (($status == '20511' || $status == '20501') && $jenis == 'draft')) {
	    ...
	    $arrdata ['subJudul'] = '- OBAT TRADISIONAL';
	}
	else
	    $arrdata ['act'] = site_url() . "/penandaan/penandaanController/setStatus/1";
...
//   }
//   $query = "SELECT DISTINCT TP.PENANDAAN_ID, TP.BBPOM_ID,  (REPLACE(REPLACE(MB.NAMA_BBPOM,'BALAI BESAR POM DI ', ''),'BALAI POM DI ','')) AS BBPOM, MS.NAMA_SARANA, MS.ALAMAT_1, CONVERT(VARCHAR(10), TP.TANGGAL_MULAI, 105) AS TANGGAL_MULAI, CONVERT(VARCHAR(10), TP.TANGGAL_AKHIR, 105) AS TANGGAL_AKHIR, TPP.NAMA_PRODUK, TPP.KLASIFIKASI_PRODUK, TPP.BENTUK_SEDIAAN, TPP.BESAR_KEMASAN, TPP.KOMPOSISI, TPP.NOMOR_IZIN_EDAR, TPP.GOLONGAN_PRODUK, TPP.PENDAFTAR FROM T_PENANDAAN TP LEFT JOIN M_BBPOM MB ON MB.BBPOM_ID = TP.BBPOM_ID LEFT JOIN M_SARANA MS ON MS.SARANA_ID = TP.SARANA_ID RIGHT JOIN T_PENANDAAN_PRODUK TPP ON TPP.PENANDAAN_ID = TP.PENANDAAN_ID RIGHT JOIN T_PENANDAAN_OBAT TPO ON TPO.PENANDAAN_ID = TP.PENANDAAN_ID  WHERE $filter2 AND TP.KOMODITI = '001' GROUP BY MB.BBPOM_ID, MB.NAMA_BBPOM, TP.KOMODITI, MS.NAMA_SARANA, MS.ALAMAT_1, TP.PENANDAAN_ID, TP.BBPOM_ID, TP.TANGGAL_MULAI, TP.TANGGAL_AKHIR, TPP.NAMA_PRODUK, TPP.KLASIFIKASI_PRODUK, TPP.BENTUK_SEDIAAN, TPP.BESAR_KEMASAN, TPP.KOMPOSISI, TPP.NOMOR_IZIN_EDAR, TPP.GOLONGAN_PRODUK, TPP.PENDAFTAR ORDER BY 1";
//   die($query);
...
//   $this->newphpexcel->mergecell(array(array('A6', 'A7'), array('B6', 'B7'), array('C6', 'C7'), array('D6', 'D7'), array('E6', 'E7'), array('F6', 'F7'), array('G6', 'G7'), array('H6', 'H7'), array('I6', 'I7'), array('J6', 'J7'), array('K6', 'K7'), array('L6', 'L7'), array('M6', 'M7'), array('N6', 'N7'), array('O6', 'O7'), array('P6', 'P7'), array('Q6', 'Q7'), array('R6', 'R7'), array('S6', 'S7'), array('T6', 'T7')), TRUE);
...
//   $this->newphpexcel->setActiveSheetIndex(0)->setCellValue('A6', 'No')->setCellValue('B6', 'Jenis Penandaan')->setCellValue('C6', 'Nama Sarana')->setCellValue('D6', 'Alamat Sarana')->setCellValue('E6', 'Tanggal Periksa')->setCellValue('F6', 'Nama Produk')->setCellValue('G6', 'Nomor Izin Edar')->setCellValue('H6', 'Klasifikasi Obat')->setCellValue('I6', 'Komposisi')->setCellValue('J6', 'Nama Pemilik Izin Edar')->setCellValue('K6', 'Bentuk Sediaan')->setCellValue('L6', 'Besar Kemasan')->setCellValue('M6', 'Golongan Obat')->setCellValue('N6', 'Uraian Pelanggaran')->setCellValue('O6', 'Hasil Balai')->setCellValue('P6', 'Hasil Pusat')->setCellValue('Q6', 'Kategori Penilaian')->setCellValue('R6', 'Tindak Lanjut Pusat')->setCellValue('S6', 'Justifikasi Pusat')->setCellValue('T6', 'Unit / Balai Pemeriksa');
//   $this->newphpexcel->headings(array('A6', 'B6', 'C6', 'D6', 'E6', 'F6', 'G6', 'H6', 'I6', 'J6', 'K6', 'L6', 'M6', 'N6', 'O6', 'P6', 'Q6', 'R6', 'S6', 'T6'));
//   $this->newphpexcel->headings(array('A7', 'B7', 'C7', 'D7', 'E7', 'F7', 'G7', 'H7', 'I7', 'J7', 'K7', 'L7', 'M7', 'N7', 'O7', 'P7', 'Q7', 'R7', 'S7', 'T7'));
//   $this->newphpexcel->set_wrap(array('A6', 'B6', 'C6', 'D6', 'E6', 'F6', 'G6', 'H6', 'I6', 'J6', 'K6', 'L6', 'M6', 'N6', 'O6', 'P6', 'Q6', 'R6', 'S6', 'T6'));
//   $this->newphpexcel->width(array(array('A', 4), array('B', 28), array('C', 30), array('D', 50), array('E', 27), array('F', 27), array('G', 27), array('H', 27), array('I', 27), array('J', 27), array('K', 27), array('L', 27), array('M', 27), array('N', 50), array('O', 15), array('P', 15), array('Q', 20), array('R', 30), array('S', 30), array('T', 33)));
//   $this->newphpexcel->set_bold(array('A1', 'A2', 'A3', 'A4'));
//   $data = $sipt->main->get_result($query);
//
//   if ($data) {
...
// }
	...
	if ($this->newsession->userdata('LOGGED_IN') == TRUE) {
		...
		$arrTLPusat = $this->config->item('tl_pusat_penandaan_ot');
		$nonTMK = array(5, 6, 12);
		$data = $sipt->main->get_result($query);
		if ($data) {
		    $no = 1;
			...
			if ($this->input->post('HASIL') != "") {
			    ...
			    $filterHasil = " AND (((TP.BBPOM_ID IN (" . "'" . join("','", $this->config->item('cfg_unit')) . "'" . ") OR TP.STATUS = 60510) AND TPO.PUSAT LIKE '%$filterMKTMK%') OR (TP.BBPOM_ID NOT IN (" . "'" . join("','", $this->config->item('cfg_unit')) . "'" . ") AND TP.STATUS <> 60510 AND TPO.SISTEM = '$filterMKTMK')) ";
			} else {
			    $filterHasil = "";
			}
			if ($mode == "ALL") {
			    $query1 = "SELECT TPO.PENILAIANBL, TPO.PENILAIANKP, TPO.URAIANBL, TPO.URAIANKP, TPO.PUSAT, TPO.SISTEM, TPO.TL_BALAI FROM T_PENANDAAN_OT TPO LEFT JOIN T_PENANDAAN TP ON TP.PENANDAAN_ID = TPO.PENANDAAN_ID WHERE TP.PENANDAAN_ID = '" . $row['PENANDAAN_ID'] . "' $filterHasil";
			} else if ($mode == "ONE") {
			    $query1 = "SELECT TPO.PENILAIAN$filterQueryAs[1] AS PENILAIAN, TPO.URAIAN$filterQueryAs[1] AS URAIAN, TPO.PUSAT, TPO.SISTEM, TPO.TL_BALAI FROM T_PENANDAAN_OT TPO LEFT JOIN T_PENANDAAN TP ON TP.PENANDAAN_ID = TPO.PENANDAAN_ID WHERE TP.PENANDAAN_ID = '" . $row['PENANDAAN_ID'] . "' $filterHasil";
			}
	...
	if ($this->newsession->userdata('LOGGED_IN') == TRUE) {
	    ...
	    $query = "SELECT DISTINCT (REPLACE(REPLACE(MB.NAMA_BBPOM,'BALAI BESAR POM DI ', ''),'BALAI POM DI ','')) AS NAMA_BBPOM,dbo.GET_KOMODITI(TP.KOMODITI) AS KOMODITI,(SUM(CASE WHEN (TPO.PUSAT LIKE '%TMK*%' AND (TP.BBPOM_ID = 92 OR TP.STATUS = 60510)) THEN 1 ELSE 0 END)) AS TMK_P,(SUM(CASE WHEN (TPO.PUSAT LIKE 'MK*%' AND (TP.BBPOM_ID = 92 OR TP.STATUS = 60510)) THEN 1 ELSE 0 END)) AS MK_P,(SUM(CASE WHEN (TPO.SISTEM = 'TMK' AND TP.BBPOM_ID <> 92 AND TP.STATUS <> 60510) THEN 1 ELSE 0 END)) AS TMK_B,(SUM(CASE WHEN (TPO.SISTEM = 'MK' AND TP.BBPOM_ID <> 92 AND TP.STATUS <> 60510) THEN 1 ELSE 0 END)) AS MK_B,(SUM(CASE WHEN TPO.PUSAT LIKE '%*0%' THEN 1 ELSE 0 END)) AS 'PERINGATAN1',(SUM(CASE WHEN TPO.PUSAT LIKE '%*1%' THEN 1 ELSE 0 END)) AS 'PERINGATAN2',(SUM(CASE WHEN TPO.PUSAT LIKE '%*2%' THEN 1 ELSE 0 END)) AS 'KERAS',(SUM(CASE WHEN TPO.PUSAT LIKE '%*3%' THEN 1 ELSE 0 END)) AS 'HENTI',(SUM(CASE WHEN TPO.PUSAT LIKE '%*4%' THEN 1 ELSE 0 END)) AS 'BATAL',(SUM(CASE WHEN TPO.TL_BALAI = '0' THEN 1 ELSE 0 END)) AS 'PERINGATANB',(SUM(CASE WHEN TPO.TL_BALAI = '1' THEN 1 ELSE 0 END)) AS 'MUSNAH' FROM T_PENANDAAN TP LEFT JOIN T_PENANDAAN_OT TPO ON TPO.PENANDAAN_ID = TP.PENANDAAN_ID LEFT JOIN M_BBPOM MB ON TP.BBPOM_ID = MB.BBPOM_ID WHERE $filter2 AND TP.KOMODITI = '010' GROUP BY  MB.BBPOM_ID, MB.NAMA_BBPOM, TP.KOMODITI ORDER BY 1";
	    $this->newphpexcel->set_font('Calibri', 10);
	    $this->newphpexcel->setActiveSheetIndex(0);
	    ...
	    $this->newphpexcel->set_bold(array('A1', 'A2', 'A3', 'A4'));
	    $this->newphpexcel->setActiveSheetIndex(0)->setCellValue('A1', 'REKAPITULASI HASIL PELAKSANAAN KEGIATAN - PENGAWASAN PENANDAAN - OBAT TRADISIONAL')->setCellValue('A2', $judul)->setCellValue('A3', strtoupper($balai))->setCellValue('A4', 'PERIODE : ' . $awal . ' s.d ' . $akhir);
