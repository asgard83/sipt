    ...
    function getListFormPenandaan($jenisPelaporan = "", $user = "") {
        if ($this->newsession->userdata('LOGGED_IN') == TRUE) {
            ...
            if (in_array('2', $this->newsession->userdata('SESS_KODE_ROLE')) && $jenisTxt == 'draft' && $user == '2') {
				...
				else $where = "A.STATUS = '".$status."' AND A.CREATE_BY = '".$this->newsession->userdata('SESS_USER_ID')."'";
            } 
			#Supervisor Satu
            ...
            $this->newtable->search(array(
                array("(CASE WHEN A.KOMODITI = '013' THEN '<div>'+dbo.GET_KOMODITI(A.KOMODITI)+'<b> &raquo '+
(SELECT JENIS + ' (' + JENIS_PENGAWASAN + ')' FROM T_PENANDAAN_PANGAN WHERE PENANDAAN_ID = A.PENANDAAN_ID)+'</b></div>' ELSE dbo.GET_KOMODITI(A.KOMODITI) END)", 'Komoditi'),
                array('B.NAMA_PRODUK', 'Nama Produk'),
                array('CONVERT(VARCHAR(10), A.TANGGAL_MULAI, 120)', 'Tanggal Awal Periksa'),
                array('CONVERT(VARCHAR(10), A.TANGGAL_AKHIR, 120)', 'Tanggal Akhir Periksa'),
                array('{IN}A.PENANDAAN_ID IN (SELECT Y.PENANDAAN_ID FROM T_PENANDAAN Y WHERE Y.PENANDAAN_ID IN(SELECT TSTP.SURAT_ID FROM T_SURAT_TUGAS_PENANDAAN TSTP WHERE TSTP.SURAT_ID = Y.SURAT_ID AND TSTP.NOMOR_SURAT {LIKE}))', 'Nomor Surat Tugas')
            ));
				$query .= "SELECT  A.LAST_UPDATE AS LASTUPDATEROW, (CAST(A.PENANDAAN_ID AS VARCHAR)+'/'+A.KOMODITI+'/'+CAST(A.STATUS AS VARCHAR))+'/$jenisPelaporan/$user'+'/'+CAST(A.BBPOM_ID AS VARCHAR) AS PENANDAANID,
				B.NAMA_PRODUK+'<div>'+B.BENTUK_SEDIAAN+'</div><div>'+B.PRODUSEN+'</div><div><b><a href=\"#\" class=\"row_preview\">Lihat Data</a></b><div>' AS 'PENGAWASAN PENANDAAN',
				CONVERT(VARCHAR(10), A.TANGGAL_MULAI, 120)+'<div>Sampai Dengan</div><div>'+CONVERT(VARCHAR(10), A.TANGGAL_AKHIR, 120)+'</div>' AS 'TANGGAL PENGAWASAN',
				'<div><b>'+E.NAMA_BBPOM+'</b></div>' AS 'BB / BPOM',
				(CASE WHEN A.KOMODITI = '013' THEN '<div>'+dbo.GET_KOMODITI(A.KOMODITI)+'<b> &raquo '+ (SELECT JENIS + ' (' + JENIS_PENGAWASAN + ')' FROM T_PENANDAAN_PANGAN WHERE PENANDAAN_ID = A.PENANDAAN_ID)+'</b></div>' ELSE dbo.GET_KOMODITI(A.KOMODITI) END) AS KOMODITI,
				C.URAIAN AS 'STATUS TERAKHIR', '<div><b>'+dbo.GET_PROPINSI(D.PROPINSI)+'</b></div>' AS PROPINSI,
				dbo.GET_HISTORY_PENANDAAN('PENGAWASAN', A.PENANDAAN_ID)+'<div>'+dbo.GET_HISTORY_PENANDAAN('CATATAN', A.PENANDAAN_ID)+'</div><div><a href=\"#\" class=\"row_riwayat2\" record=\"'+CAST(A.PENANDAAN_ID AS VARCHAR)+'#T_PENANDAAN_PROSES\">Riwayat Pemeriksaan</a></div>' AS [UPDATE TERAKHIR]
				FROM T_PENANDAAN A LEFT JOIN T_PENANDAAN_PRODUK B ON A.PENANDAAN_ID = B.PENANDAAN_ID
				LEFT JOIN M_TABEL C ON A.STATUS = C.KODE LEFT JOIN M_SARANA D ON A.SARANA_ID = D.SARANA_ID
				LEFT JOIN M_BBPOM E ON A.BBPOM_ID = E.BBPOM_ID
				WHERE ".$where." AND A.KOMODITI IN(" . "'" . join("','", $this->newsession->userdata("SESS_KLASIFIKASI_ID")) . "'" . ")";
            ...
            if (!in_array($this->newsession->userdata('SESS_BBPOM_ID'), $this->config->item('cfg_unit'))) {
				...
				//if ($jenisPelaporan == "1212") {
					//$query .= "AND A.USER_LAST_UPDATE = '".$this->newsession->userdata('SESS_USER_ID') . "'";
				//}
            }
            if ($jenisPelaporan == "1222" && in_array('6', $this->newsession->userdata('SESS_KODE_ROLE'))) {
                $query .= " AND A.BBPOM_ID = '92'";
                $subJudul = 'Data Pusat';
            }
            ...
            $this->newtable->columns(array(
                "A.LAST_UPDATE", 
				"(CAST(A.PENANDAAN_ID AS VARCHAR)+'/'+A.KOMODITI+'/'+CAST(A.STATUS AS VARCHAR))+'/$jenisPelaporan/$user'+'/'+CAST(A.BBPOM_ID AS VARCHAR)", 
				"B.NAMA_PRODUK+'<div>'+B.BENTUK_SEDIAAN+'</div><div>'+B.PRODUSEN+'</div><div><b><a href=\"#\" class=\"row_preview\">Lihat Data</a></b><div>'", 
				"CONVERT(VARCHAR(10), A.TANGGAL_MULAI, 120)+'<div>Sampai Dengan</div><div>'+CONVERT(VARCHAR(10), A.TANGGAL_AKHIR, 120)+'</div>'",
				"'<div><b>'+E.NAMA_BBPOM+'</b></div>'",
				"(CASE WHEN A.KOMODITI = '013' THEN '<div>'+dbo.GET_KOMODITI(A.KOMODITI)+'<b> &raquo '+ (SELECT JENIS + ' (' + JENIS_PENGAWASAN + ')' FROM T_PENANDAAN_PANGAN WHERE PENANDAAN_ID = A.PENANDAAN_ID)+'</b></div>' ELSE dbo.GET_KOMODITI(A.KOMODITI) END)",
				"C.URAIAN", "'<div><b>'+dbo.GET_PROPINSI(D.PROPINSI)+'</b></div>'",
				"dbo.GET_HISTORY_PENANDAAN('PENGAWASAN', A.PENANDAAN_ID)+'<div>'+dbo.GET_HISTORY_PENANDAAN('CATATAN', A.PENANDAAN_ID)+'</div><div><a href=\"#\" class=\"row_riwayat2\" record=\"'+CAST(A.PENANDAAN_ID AS VARCHAR)+'#T_PENANDAAN_PROSES\">Riwayat Pemeriksaan</a></div>'"
            ));
            ...
            $this->newtable->menu($proses); //echo $query;
