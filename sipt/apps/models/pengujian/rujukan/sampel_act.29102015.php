...
class Sampel_act extends Model{
	...
	function get_sampel($id){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			$arrstatus = array('80203','50205','80205','50203','80011','80215');
			#$arrdata['sampel'][0]['STATUS_SAMPEL'] == '80203' || $arrdata['sampel'][0]['STATUS_SAMPEL'] == '50205' || $arrdata['sampel'][0]['STATUS_SAMPEL'] == '80205'
			#if(in_array($arrdata['sampel'][0]['STATUS_SAMPEL'],$arrstatus)){
				$query = "SELECT CASE WHEN A.JENIS_UJI = '01' THEN 'Mikrobiologi' WHEN A.JENIS_UJI = '02' THEN 'Kimia-Fisika' END AS BIDANG_UJI, A.PARAMETER_UJI, A.METODE, A.PUSTAKA, A.SYARAT, B.URAIAN AS LINGKUP_UJI, REPLACE(REPLACE(C.NAMA_BBPOM, 'BALAI POM DI ', ''), 'BALAI BESAR POM DI ','') AS [BALAI_TUJUAN], A.LCP FROM T_SAMPEL_RUJUKAN_DETIL A LEFT JOIN M_LINGKUP_RUJUKAN B ON A.LINGKUP_UJI = B.ID LEFT JOIN M_BBPOM C ON A.BBPOM_ID = C.BBPOM_ID WHERE A.KODE_SAMPEL = '".$id."'";
				if($arrdata['sampel'][0]['STATUS_SAMPEL']=='50205'){
					$arrdata['actkabalai'] = TRUE;
					$qkode = "SELECT KODE_SAMPEL FROM T_M_SAMPEL WHERE KODE_RUJUKAN = '".$id."'";
			...
			/*if($status == '50205'){#Hasil Akhir Pengujian
				...
				$kode_sampel = $sipt->main->get_uraian("SELECT KODE_SAMPEL FROM T_M_SAMPEL WHERE KODE_RUJUKAN = '".$id."' AND SUBSTRING(KODE_SAMPEL,3,3) = '".$substr."'","KODE_SAMPEL"); 
				...
				$arrdata['dtrujukan'] = $this->db->query("SELECT CATATAN FROM T_SAMPEL_RUJUKAN WHERE KODE_SAMPEL = '".$id."' AND BBPOM_RUJUK = '".$this->newsession->userdata('SESS_BBPOM_ID')."'")->result_array();
			}*/
