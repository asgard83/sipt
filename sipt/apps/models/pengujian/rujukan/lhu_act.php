	...
	function list_lhu($id){
			if((in_array('3', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('4', $this->newsession->userdata('SESS_KODE_ROLE')) && in_array('02', $this->newsession->userdata('SESS_JENIS_PELAPORAN'))) && $this->newsession->userdata('LOGGED_IN') ==  TRUE){
				...
				if($id == "draft"){
					...
					$query = "SELECT B.CP_ID, A.KODE_SAMPEL, B.STATUS, C.USER_ID, dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL) AS [KODE SAMPEL], A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>' AS [IDENTITAS SAMPEL], dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + dbo.KATEGORI(A.KATEGORI,A.PRIORITAS) + '</div>' AS KOMODITI, C.NAMA_USER AS [PENYELIA] FROM T_M_SAMPEL A LEFT JOIN T_CP B ON A.KODE_SAMPEL = B.KODE_SAMPEL LEFT JOIN T_USER C ON B.CREATE_BY = C.USER_ID WHERE B.BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND B.MT = '".$this->newsession->userdata('SESS_USER_ID')."' AND B.STATUS = '40202'";
					$this->newtable->columns(array("B.CP_ID", "A.KODE_SAMPEL", "B.STATUS", "C.USER_ID", array("dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL)",site_url()."/home/pengujian/lhu/new/{CP_ID}.{KODE_SAMPEL}.{STATUS}.{USER_ID}"), "A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>'", "dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + dbo.KATEGORI(A.KATEGORI,A.PRIORITAS) + '</div>'","C.NAMA_USER"));
					...
					if(in_array('3', $this->newsession->userdata('SESS_KODE_ROLE'))){
						$query = "SELECT B.CP_ID, A.KODE_SAMPEL, B.STATUS, C.USER_ID, dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL) AS [KODE SAMPEL], A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>' AS [IDENTITAS SAMPEL], dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + dbo.KATEGORI(A.KATEGORI,A.PRIORITAS) + '</div>' AS KOMODITI, C.NAMA_USER AS [PENYELIA] FROM T_M_SAMPEL A LEFT JOIN T_CP B ON A.KODE_SAMPEL = B.KODE_SAMPEL  LEFT JOIN T_USER C ON B.CREATE_BY = C.USER_ID WHERE B.BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND B.CREATE_BY = '".$this->newsession->userdata('SESS_USER_ID')."' AND B.STATUS IN('40204','50202')";
						$proses['Cetak Data LHU'] = array('GETNEW', site_url()."/topdf/lhu/prints", '1');
					}
					if(in_array('4', $this->newsession->userdata('SESS_KODE_ROLE'))){
						$query = "SELECT B.CP_ID, A.KODE_SAMPEL, B.STATUS, C.USER_ID, dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL) AS [KODE SAMPEL], A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>' AS [IDENTITAS SAMPEL], dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + dbo.KATEGORI(A.KATEGORI,A.PRIORITAS) + '</div>' AS KOMODITI,  C.NAMA_USER AS [PENYELIA] FROM T_M_SAMPEL A LEFT JOIN T_CP B ON A.KODE_SAMPEL = B.KODE_SAMPEL LEFT JOIN T_USER C ON B.CREATE_BY = C.USER_ID WHERE B.BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND B.MT = '".$this->newsession->userdata('SESS_USER_ID')."' AND B.STATUS = '40204'";
					}
					$this->newtable->columns(array("B.CP_ID", "A.KODE_SAMPEL", "B.STATUS", "C.USER_ID", array("dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL)",site_url()."/home/pengujian/lhu/konsep/{CP_ID}.{KODE_SAMPEL}.{STATUS}.{USER_ID}"), "A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>'", "dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + dbo.KATEGORI(A.KATEGORI,A.PRIORITAS) + '</div>'","C.NAMA_USER"));
					...
					$judul = "Laporan Hasil Uji";
					$query = "SELECT B.CP_ID, A.KODE_SAMPEL, B.STATUS, C.USER_ID, dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL) AS [KODE SAMPEL], A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>' AS [IDENTITAS SAMPEL], dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + dbo.KATEGORI(A.KATEGORI,A.PRIORITAS) + '</div>' AS KOMODITI, C.NAMA_USER AS [PENYELIA], CASE WHEN A.STATUS_KIMIA = 1 AND A.STATUS_MIKRO IS NULL THEN '&radic; Kimia - &times; Mikro' WHEN A.STATUS_KIMIA IS NULL AND A.STATUS_MIKRO = 1 THEN '&times; Kimia - &radic; Mikro' WHEN A.STATUS_KIMIA = 1 AND A.STATUS_MIKRO = 1 THEN '&radic; Kimia - &radic; Mikro' ELSE '&times; Kimia - &times; Mikro' END AS [STATUS UJI] FROM T_M_SAMPEL A LEFT JOIN T_CP B ON A.KODE_SAMPEL = B.KODE_SAMPEL LEFT JOIN T_USER C ON B.CREATE_BY = C.USER_ID WHERE B.BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND B.CREATE_BY = '".$this->newsession->userdata('SESS_USER_ID')."' AND B.STATUS = '30203'";
					
					$this->newtable->columns(array("B.CP_ID","A.KODE_SAMPEL", "B.STATUS", "C.USER_ID", array("dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL)",site_url()."/home/pengujian/lhu/konsep/{CP_ID}.{KODE_SAMPEL}.{STATUS}"), "A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>'", "dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + dbo.KATEGORI(A.KATEGORI,A.PRIORITAS) + '</div>'","C.NAMA_USER","CASE WHEN A.STATUS_KIMIA = 1 AND A.STATUS_MIKRO IS NULL THEN '&radic; Kimia - &times; Mikro' WHEN A.STATUS_KIMIA IS NULL AND A.STATUS_MIKRO = 1 THEN '&radic; Kimia - &times; Mikro' WHEN A.STATUS_KIMIA = 1 AND A.STATUS_MIKRO = 1 THEN '&radic; Kimia - &radic; Mikro' ELSE '&times; Kimia - &times; Mikro' END"));
					...
					$query = "SELECT B.CP_ID, A.KODE_SAMPEL, B.STATUS, C.USER_ID, dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL) AS [KODE SAMPEL], A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>' AS [IDENTITAS SAMPEL], dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + dbo.KATEGORI(A.KATEGORI,A.PRIORITAS) + '</div>' AS KOMODITI, C.NAMA_USER AS [PENYELIA], CONVERT(VARCHAR(10), B.LAST_UPDATE, 120) AS LAST_UPDATE FROM T_M_SAMPEL A LEFT JOIN T_CP B ON A.KODE_SAMPEL = B.KODE_SAMPEL LEFT JOIN T_USER C ON B.CREATE_BY = C.USER_ID WHERE B.BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND B.MT = '".$this->newsession->userdata('SESS_USER_ID')."' AND B.STATUS = '50202'";
					$this->newtable->columns(array("B.CP_ID","A.KODE_SAMPEL", "B.STATUS", "C.USER_ID", array("dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL)",site_url()."/home/pengujian/lhu/konsep/{CP_ID}.{KODE_SAMPEL}.{STATUS}"), "A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>'", "dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + dbo.KATEGORI(A.KATEGORI,A.PRIORITAS) + '</div>'","C.NAMA_USER","CONVERT(VARCHAR(10), B.LAST_UPDATE, 120)"));
					$proses['Preview Data'] = array('GET', site_url()."/home/pengujian/lhu/konsep", '1');
	...
	function get_lhu($id){
		if((in_array('3', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('4', $this->newsession->userdata('SESS_KODE_ROLE')) && in_array('02', $this->newsession->userdata('SESS_JENIS_PELAPORAN'))) && $this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			if($arrid[2] == "40202"){
				...
				//$arrdata['bbpom'] = $sipt->main->combobox("SELECT BBPOM_ID AS ID,NAMA_BBPOM AS URAIAN FROM  M_BBPOM WHERE BBPOM_ID IN (SELECT BBPOM_TUJUAN FROM M_LINGKUP_RUJUKAN_DETIL)", "ID", "URAIAN", TRUE);				
			}
