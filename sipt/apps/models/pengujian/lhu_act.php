	...
	function list_lhu($id){
		if((in_array('3', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('4', $this->newsession->userdata('SESS_KODE_ROLE')) && in_array('02', $this->newsession->userdata('SESS_JENIS_PELAPORAN'))) && $this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			if($id == "draft"){
				...
				$query = "SELECT B.CP_ID, A.KODE_SAMPEL, B.STATUS, C.USER_ID, dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL) AS [KODE SAMPEL], A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>' AS [IDENTITAS SAMPEL], dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + A.NAMA_KATEGORI + '</div>' AS KOMODITI, C.NAMA_USER AS [PENYELIA], CASE WHEN A.UJI_RUJUK = 1 THEN 'Uji Rujuk' WHEN A.UJI_UNGGULAN = 1 THEN 'Uji Unggulan' ELSE 'UjI Rutin' END AS [TIPE UJI] FROM T_M_SAMPEL A LEFT JOIN T_CP B ON A.KODE_SAMPEL = B.KODE_SAMPEL LEFT JOIN T_USER C ON B.CREATE_BY = C.USER_ID WHERE B.BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND B.MT = '".$this->newsession->userdata('SESS_USER_ID')."' AND B.STATUS = '40202'";
				$this->newtable->columns(array("B.CP_ID", "A.KODE_SAMPEL", "B.STATUS", "C.USER_ID", array("dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL)",site_url()."/home/pengujian/lhu/new/{CP_ID}.{KODE_SAMPEL}.{STATUS}.{USER_ID}"), "A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>'", "dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + A.NAMA_KATEGORI + '</div>'","C.NAMA_USER", "CASE WHEN A.UJI_RUJUK = 1 THEN 'Uji Rujuk' WHEN A.UJI_UNGGULAN = 1 THEN 'Uji Unggulan' ELSE 'UjI Rutin' END"));
				...
				if(in_array('3', $this->newsession->userdata('SESS_KODE_ROLE'))){
					$query = "SELECT B.CP_ID, A.KODE_SAMPEL, B.STATUS, C.USER_ID, dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL) AS [KODE SAMPEL], A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>' AS [IDENTITAS SAMPEL], dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + A.NAMA_KATEGORI + '</div>' AS KOMODITI, C.NAMA_USER AS [PENYELIA], 
						CASE WHEN A.UJI_RUJUK = 1 THEN 'Uji Rujuk' WHEN A.UJI_UNGGULAN = 1 THEN 'Uji Unggulan' ELSE 'UjI Rutin' END AS [TIPE UJI] FROM T_M_SAMPEL A LEFT JOIN T_CP B ON A.KODE_SAMPEL = B.KODE_SAMPEL  LEFT JOIN T_USER C ON B.CREATE_BY = C.USER_ID WHERE B.BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND B.CREATE_BY = '".$this->newsession->userdata('SESS_USER_ID')."' AND B.STATUS IN('40204','50202')";
					$proses['Cetak Data LHU'] = array('GETNEW', site_url()."/topdf/lhu/prints", '1');
				}
				if(in_array('4', $this->newsession->userdata('SESS_KODE_ROLE'))){
					$query = "SELECT B.CP_ID, A.KODE_SAMPEL, B.STATUS, C.USER_ID, dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL) AS [KODE SAMPEL], A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>' AS [IDENTITAS SAMPEL], dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + A.NAMA_KATEGORI + '</div>' AS KOMODITI,  C.NAMA_USER AS [PENYELIA], CASE WHEN A.UJI_RUJUK = 1 THEN 'Uji Rujuk' WHEN A.UJI_UNGGULAN = 1 THEN 'Uji Unggulan' ELSE 'UjI Rutin' END AS [TIPE UJI] FROM T_M_SAMPEL A LEFT JOIN T_CP B ON A.KODE_SAMPEL = B.KODE_SAMPEL LEFT JOIN T_USER C ON B.CREATE_BY = C.USER_ID WHERE B.BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND B.MT = '".$this->newsession->userdata('SESS_USER_ID')."' AND B.STATUS = '40204'";
				}
				$this->newtable->columns(array("B.CP_ID", "A.KODE_SAMPEL", "B.STATUS", "C.USER_ID", array("dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL)",site_url()."/home/pengujian/lhu/konsep/{CP_ID}.{KODE_SAMPEL}.{STATUS}.{USER_ID}"), "A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>'", "dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + A.NAMA_KATEGORI + '</div>'","C.NAMA_USER","CASE WHEN A.UJI_RUJUK = 1 THEN 'Uji Rujuk' WHEN A.UJI_UNGGULAN = 1 THEN 'Uji Unggulan' ELSE 'UjI Rutin' END"));
				...
				$judul = "Laporan Hasil Uji";
				$query = "SELECT B.CP_ID, A.KODE_SAMPEL, B.STATUS, C.USER_ID, dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL) AS [KODE SAMPEL], A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>' AS [IDENTITAS SAMPEL], dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + A.NAMA_KATEGORI + '</div>' AS KOMODITI, C.NAMA_USER AS [PENYELIA], CASE WHEN A.STATUS_KIMIA = 1 AND (A.STATUS_MIKRO IS NULL OR A.STATUS_MIKRO = 0) THEN '&radic; Kimia - &times; Mikro' WHEN (A.STATUS_KIMIA IS NULL OR A.STATUS_KIMIA = 0) AND A.STATUS_MIKRO = 1 THEN '&times; Kimia - &radic; Mikro' WHEN A.STATUS_KIMIA = 1 AND A.STATUS_MIKRO = 1 THEN '&radic; Kimia - &radic; Mikro' ELSE '&times; Kimia - &times; Mikro' END AS [STATUS UJI], CASE WHEN A.UJI_RUJUK = 1 THEN 'Uji Rujuk' WHEN A.UJI_UNGGULAN = 1 THEN 'Uji Unggulan' ELSE 'UjI Rutin' END AS [TIPE UJI] FROM T_M_SAMPEL A LEFT JOIN T_CP B ON A.KODE_SAMPEL = B.KODE_SAMPEL LEFT JOIN T_USER C ON B.CREATE_BY = C.USER_ID WHERE B.BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND B.CREATE_BY = '".$this->newsession->userdata('SESS_USER_ID')."' AND B.STATUS = '30203'";
				
				$this->newtable->columns(array("B.CP_ID","A.KODE_SAMPEL", "B.STATUS", "C.USER_ID", array("dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL)",site_url()."/home/pengujian/lhu/konsep/{CP_ID}.{KODE_SAMPEL}.{STATUS}"), "A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>'", "dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + A.NAMA_KATEGORI + '</div>'","C.NAMA_USER","CASE WHEN A.STATUS_KIMIA = 1 AND (A.STATUS_MIKRO IS NULL OR A.STATUS_MIKRO = 0) THEN '&radic; Kimia - &times; Mikro' WHEN (A.STATUS_KIMIA IS NULL OR A.STATUS_KIMIA = 0) AND A.STATUS_MIKRO = 1 THEN '&times; Kimia - &radic; Mikro' WHEN A.STATUS_KIMIA = 1 AND A.STATUS_MIKRO = 1 THEN '&radic; Kimia - &radic; Mikro' ELSE '&times; Kimia - &times; Mikro' END", "CASE WHEN A.UJI_RUJUK = 1 THEN 'Uji Rujuk' WHEN A.UJI_UNGGULAN = 1 THEN 'Uji Unggulan' ELSE 'UjI Rutin' END"));
				...
				$query = "SELECT B.CP_ID, A.KODE_SAMPEL, B.STATUS, C.USER_ID, dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL) AS [KODE SAMPEL], A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>' AS [IDENTITAS SAMPEL], dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + A.NAMA_KATEGORI + '</div>' AS KOMODITI, C.NAMA_USER AS [PENYELIA], CASE WHEN A.UJI_RUJUK = 1 THEN 'Uji Rujuk' WHEN A.UJI_UNGGULAN = 1 THEN 'Uji Unggulan' ELSE 'UjI Rutin' END AS [TIPE UJI], CONVERT(VARCHAR(10), B.LAST_UPDATE, 120) AS LAST_UPDATE FROM T_M_SAMPEL A LEFT JOIN T_CP B ON A.KODE_SAMPEL = B.KODE_SAMPEL LEFT JOIN T_USER C ON B.CREATE_BY = C.USER_ID WHERE B.BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND B.MT = '".$this->newsession->userdata('SESS_USER_ID')."' AND B.STATUS = '50202'";
				$this->newtable->columns(array("B.CP_ID","A.KODE_SAMPEL", "B.STATUS", "C.USER_ID", array("dbo.FORMAT_NOMOR('SPL', A.KODE_SAMPEL)",site_url()."/home/pengujian/lhu/konsep/{CP_ID}.{KODE_SAMPEL}.{STATUS}"), "A.NAMA_SAMPEL + '<div>' + A.KEMASAN + '</div><div>' + A.NOMOR_REGISTRASI + '</div>'", "dbo.KATEGORI(A.KOMODITI,A.PRIORITAS) + '<div>' + A.NAMA_KATEGORI + '</div>'","C.NAMA_USER","CASE WHEN A.UJI_RUJUK = 1 THEN 'Uji Rujuk' WHEN A.UJI_UNGGULAN = 1 THEN 'Uji Unggulan' ELSE 'UjI Rutin' END","CONVERT(VARCHAR(10), B.LAST_UPDATE, 120)"));
				$proses['Preview Data'] = array('GET', site_url()."/home/pengujian/lhu/konsep", '1');
				$this->newtable->orderby(7);
				$this->newtable->sortby("ASC");
				$this->newtable->hiddens(array('CP_ID','KODE_SAMPEL','STATUS', 'USER_ID','LAST_UPDATE'));
			}
			$this->newtable->search(array(array("dbo.FORMAT_NOMOR('SPL', B.KODE_SAMPEL)", "Kode Sampel"),array("A.NAMA_SAMPEL","Nama Sampel"), array("dbo.KATEGORI(A.KOMODITI,A.PRIORITAS)","Komoditi"), array("CASE WHEN A.UJI_RUJUK = 1 THEN 'Uji Rujuk' WHEN A.UJI_UNGGULAN = 1 THEN 'Uji Unggulan' ELSE 'UjI Rutin' END", "Tipe Uji")));
			...
			$this->newtable->width(array('KODE SAMPEL' => 75, 'IDENTITAS SAMPEL' => 300, 'KOMODITI' => 200, 'PENYELIA' => 100, 'TIPE UJI' => 50));
					...
					for($i = 0; $i < count($_POST['RUJUKAN']); $i++){							
							...
							if($datac){
								...
								foreach($queryc->result_array() as $row){
									...
									$arrtmp['LCP'] = $row['LCP'];
									$arrtmp['JUMLAH_SAMPEL'] = $_POST['RUJUKAN']['JUMLAH_SAMPEL'][$i];
