...
class Spux_act extends Model{
	function preview($id){
		if((in_array('2', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('3', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('4', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('5', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('7', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('8', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('11', $this->newsession->userdata('SESS_KODE_ROLE'))) && ($this->newsession->userdata('SESS_BBPOM_ID') == '99' && $this->newsession->userdata('LOGGED_IN') ==  TRUE)){
			...
			$arrid = explode(".", $id);
			if(count($arrid) <= 1){
				return redirect(base_url()); die();
			}
			$qspu = "SELECT SPU_ID, dbo.FORMAT_NOMOR('SPU', SPU_ID) AS FORMAT_SPU, CONVERT(VARCHAR(10), TANGGAL, 103) AS TANGGAL, CONVERT(VARCHAR(10), TANGGAL_TERIMA_TPS, 103) AS TANGGAL_TERIMA_TPS, PETUGAS_PENERIMA, dbo.URAIAN_M_TABEL('ANGGARAN_SAMPLING', SUBSTRING(SPU_ID, 7,2)) AS ANGGARAN, dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL) AS [ASAL SAMPEL], dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2)) AS KOMODITI, STATUS FROM T_SPU WHERE SPU_ID = '".$arrid[0]."'";
			...
			if($rspu){
				...
				$arrdata['jml_log'] = $sipt->main->get_uraian("SELECT COUNT(*) AS JML FROM T_SPU_LOG WHERE SPU_ID ='".$arrid[0]."'","JML");
				...
				$arrdata['showjml'] = $sipt->main->get_uraian("SELECT COUNT(JML) AS JML FROM (SELECT DISTINCT(STATUS_SAMPEL) AS JML FROM T_M_SAMPEL WHERE SPU_ID = '".$arrid[0]."') AS DATA","JML");
				...
				$allowed = $sipt->main->get_uraian("SELECT GET_DISPO FROM T_SPU WHERE SPU_ID = '".$arrid[0]."'","GET_DISPO");
...
dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2)) AS [KOMODITI], dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL) AS [ASAL SAMPEL], CONVERT(VARCHAR(10), TANGGAL_TERIMA_TPS, 120) AS [TANGGAL TERIMA] FROM T_SPU WHERE BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' $status";
			...
			//if($menu == "all"){
				//$proses['Cetak Data SPU'] = array('GETNEW', site_url()."/topdf/ppomn/spux/prints", '1');
			//}
			$this->newtable->menu($proses);
		...
		if((in_array('7', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('8', $this->newsession->userdata('SESS_KODE_ROLE'))) && $this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			if(in_array('7', $this->newsession->userdata('SESS_KODE_ROLE'))){
				if($submenu == "verifikasi"){
					...
					$query = "SELECT SPU_ID, dbo.FORMAT_NOMOR('SPU',SPU_ID) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL, 120) +'</div>' AS [NOMOR & TANGGAL], dbo.URAIAN_M_TABEL('ANGGARAN_SAMPLING', SUBSTRING(SPU_ID, 7,2)) +'<div>'+ dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL) +'</div>' AS [ANGGARAN & ASAL SAMPEL], dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2)) AS [KOMODITI], dbo.TOTAL_SAMPEL(SPU_ID) AS [TOTAL SAMPEL], dbo.URAIAN_M_TABEL('STATUS',STATUS) AS STATUS FROM T_SPU WHERE BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND STATUS = '70000' AND ASAL_SAMPEL IN ('10','11','12')";
					$this->newtable->columns(array("SPU_ID", array("dbo.FORMAT_NOMOR('SPU',SPU_ID) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL, 120) +'</div>'",site_url()."/home/spux/preview/{SPU_ID}"), "dbo.URAIAN_M_TABEL('ANGGARAN_SAMPLING', SUBSTRING(SPU_ID, 7,2)) +'<div>'+ dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL)+'</div>'","dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2))","dbo.TOTAL_SAMPEL(SPU_ID)", "dbo.URAIAN_M_TABEL('STATUS',STATUS)"));
					$this->newtable->search(array(array("dbo.FORMAT_NOMOR('SPU',SPU_ID)", "Nomor Surat Permintaan Uji"),array("CONVERT(VARCHAR(10), TANGGAL, 120)","Tanggal Surat"),array("dbo.URAIAN_M_TABEL('ANGGARAN_SAMPLING', SUBSTRING(SPU_ID, 7,2))","Anggaran Sampel"),array("dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL)","Asal Sampel"),array("dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2))","Komoditi"),array("dbo.URAIAN_M_TABEL('STATUS',STATUS)","Status SPU")));
					...
					$query = "SELECT SPU_ID, dbo.FORMAT_NOMOR('SPU',SPU_ID) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL, 120) +'</div>' AS [NOMOR & TANGGAL SPU], dbo.FORMAT_NOMOR('SPS',NOMOR_SPS) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL_TERIMA_TPS, 120) +'</div>' AS [NOMOR & TANGGAL SPS], dbo.URAIAN_M_TABEL('ANGGARAN_SAMPLING', SUBSTRING(SPU_ID, 7,2)) +'<div>'+ dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL) +'</div>' AS [ANGGARAN & ASAL SAMPEL], dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2)) AS [KOMODITI], dbo.TOTAL_SAMPEL(SPU_ID) AS [TOTAL SAMPEL], dbo.URAIAN_M_TABEL('STATUS',STATUS) AS STATUS FROM T_SPU WHERE BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' $q  AND ASAL_SAMPEL IN ('10','11','12')";
					$this->newtable->columns(array("SPU_ID", array("dbo.FORMAT_NOMOR('SPU',SPU_ID) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL, 120) +'</div>'",site_url()."/home/spux/preview/{SPU_ID}"), "dbo.FORMAT_NOMOR('SPS',NOMOR_SPS) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL_TERIMA_TPS, 120) +'</div>'", "dbo.URAIAN_M_TABEL('ANGGARAN_SAMPLING', SUBSTRING(SPU_ID, 7,2)) +'<div>'+ dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL)+'</div>'","dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2))","dbo.TOTAL_SAMPEL(SPU_ID)", "dbo.URAIAN_M_TABEL('STATUS',STATUS)"));
					$this->newtable->search(array(array("dbo.FORMAT_NOMOR('SPU',SPU_ID)", "Nomor Surat Permintaan Uji"),array("CONVERT(VARCHAR(10), TANGGAL, 120)","Tanggal Surat"),array("dbo.FORMAT_NOMOR('SPS',NOMOR_SPS)", "Nomor Surat Penyerahan Sampel"),array("CONVERT(VARCHAR(10), TANGGAL_TERIMA_TPS, 120)","Tanggal Penerimaan Sampel"), array("dbo.URAIAN_M_TABEL('ANGGARAN_SAMPLING', SUBSTRING(SPU_ID, 7,2))","Anggaran Sampel"),array("dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL)","Asal Sampel"),array("dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2))","Komoditi"),array("dbo.URAIAN_M_TABEL('STATUS',STATUS)","Status SPU")));
				...
				if($submenu == "verifikasi"){
					$query = "SELECT SPU_ID, dbo.FORMAT_NOMOR('SPU',SPU_ID) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL, 120) +'</div>' AS [NOMOR & TANGGAL SPU], dbo.FORMAT_NOMOR('SPS',NOMOR_SPS) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL_TERIMA_TPS, 120) +'</div>' AS [NOMOR & TANGGAL SPS], dbo.URAIAN_M_TABEL('ANGGARAN_SAMPLING', SUBSTRING(SPU_ID, 7,2)) +'<div>'+ dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL) +'</div>' AS [ANGGARAN & ASAL SAMPEL], dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2)) AS [KOMODITI], dbo.TOTAL_SAMPEL(SPU_ID) AS [TOTAL SAMPEL], dbo.URAIAN_M_TABEL('STATUS',STATUS) AS STATUS FROM T_SPU WHERE BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND STATUS = '80201' AND ASAL_SAMPEL IN ('10','11','12')";
				}else{
					$query = "SELECT SPU_ID, dbo.FORMAT_NOMOR('SPU',SPU_ID) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL, 120) +'</div>' AS [NOMOR & TANGGAL SPU], dbo.FORMAT_NOMOR('SPS',NOMOR_SPS) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL_TERIMA_TPS, 120) +'</div>' AS [NOMOR & TANGGAL SPS], dbo.URAIAN_M_TABEL('ANGGARAN_SAMPLING', SUBSTRING(SPU_ID, 7,2)) +'<div>'+ dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL) +'</div>' AS [ANGGARAN & ASAL SAMPEL], dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2)) AS [KOMODITI], dbo.TOTAL_SAMPEL(SPU_ID) AS [TOTAL SAMPEL], dbo.URAIAN_M_TABEL('STATUS',STATUS) AS STATUS FROM T_SPU WHERE BBPOM_ID = '".$this->newsession->userdata('SESS_BBPOM_ID')."' AND STATUS IN ('50201') AND ASAL_SAMPEL IN ('10','11','12')";
				}
				$this->newtable->columns(array("SPU_ID", array("dbo.FORMAT_NOMOR('SPU',SPU_ID) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL, 120) +'</div>'",site_url()."/home/spux/preview/{SPU_ID}"), "dbo.FORMAT_NOMOR('SPS',NOMOR_SPS) +'<div>'+ CONVERT(VARCHAR(10), TANGGAL_TERIMA_TPS, 120) +'</div>'", "dbo.URAIAN_M_TABEL('ANGGARAN_SAMPLING', SUBSTRING(SPU_ID, 7,2)) +'<div>'+ dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL)+'</div>'","dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2))","dbo.TOTAL_SAMPEL(SPU_ID)", "dbo.URAIAN_M_TABEL('STATUS',STATUS)"));
				$this->newtable->search(array(array("dbo.FORMAT_NOMOR('SPU',SPU_ID)", "Nomor Surat Permintaan Uji"),array("CONVERT(VARCHAR(10), TANGGAL, 120)","Tanggal Surat"),array("dbo.FORMAT_NOMOR('SPS',NOMOR_SPS)", "Nomor Surat Penyerahan Sampel"),array("CONVERT(VARCHAR(10), TANGGAL_TERIMA_TPS, 120)","Tanggal Penerimaan Sampel"), array("dbo.URAIAN_M_TABEL('ANGGARAN_SAMPLING', SUBSTRING(SPU_ID, 7,2))","Anggaran Sampel"),array("dbo.URAIAN_M_TABEL('ASAL_SAMPLING', ASAL_SAMPEL)","Asal Sampel"),array("dbo.KATEGORI(SUBSTRING(SPU_ID, 13,2))","Komoditi"),array("dbo.URAIAN_M_TABEL('STATUS',STATUS)","Status SPU")));
				$this->newtable->width(array('NOMOR & TANGGAL SPU' => 125, 'NOMOR & TANGGAL SPS' => 125, 'ANGGARAN & ASAL SAMPEL' => 200, 'KOMODITI' => 75,'TOTAL SAMPEL' => 50, 'STATUS' => 150));
	...
	function set_spu($action, $isajax){
		if((in_array('7', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('8', $this->newsession->userdata('SESS_KODE_ROLE')) || in_array('11', $this->newsession->userdata('SESS_KODE_ROLE'))) && $this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			if($action=="save"){
				$hasil = FALSE;
				$msgok = "Konsep SPU berhasil dikirimkan ke bidang";
				$msgerr = "Konsep SPU gagal dikirimkan ke bidang, Silahkan coba lagi";
				$arr_petugas = $this->input->post('USER_ID');
				if(!$arr_petugas){
					return "MSG#NO#Konsep SPU gagal dikirimkan.\n Mohon diperiksa kembali tujuan Bidang Pengujian"; die();
				}
				...
				if($this->db->insert('T_SPU',$dtspu)){
					...
					if(count($arr_petugas)>0){
						foreach($arr_petugas as $user){
							$spetugas['SPU_ID'] = $dtspu['SPU_ID'];
							$spetugas['USER_ID'] = $user;
							if($this->db->insert('T_SP_PETUGAS', $spetugas)){
								$hasil = TRUE;
							}
						}
					}else{
						$hasil = FALSE;
						$this->db->simple_query("DELETE FROM T_SPU WHERE SPU_ID = '".$dtspu['SPU_ID']."'");
						$this->db->simple_query("DELETE FROM T_SP_PETUGAS WHERE SPU_ID = '".$dtspu['SPU_ID']."'");
						foreach($kode as $a){
							$this->db->simple_query("UPDATE T_M_SAMPEL SET SPU_ID ='".$dtspu['SPU_ID']."', STATUS_SAMPEL = '70000' WHERE KODE_SAMPEL = '".str_replace("'","",$a)."'");
						}
					}
				}
				if($hasil){
					...
					foreach($kode as $a){
						$this->db->simple_query("UPDATE T_M_SAMPEL SET SPU_ID ='".$dtspu['SPU_ID']."', STATUS_SAMPEL = '11001' WHERE KODE_SAMPEL = '".str_replace("'","",$a)."'");
