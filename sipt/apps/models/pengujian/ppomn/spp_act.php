...
class Spp_act extends Model{
	function get_spp($id){
		if(in_array('7', $this->newsession->userdata('SESS_KODE_ROLE')) && $this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			if($query){
				...
				$this->newtable->action(site_url()."home/ppomn/spu/".$id);
		...
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			if($action == "save" && in_array('7', $this->newsession->userdata('SESS_KODE_ROLE'))){
				...
				if($hasil){
					...
					return "MSG#YES#$msgok.#".site_url().'/home/ppomn/sps/list/all';
				}else{
				...
				if($hasil){
					return "MSG#YES#$msgok.#".site_url().'/home/ppomn/spp/view/'.$this->input->post('SPK_ID');
				}else{
					return "MSG#NO#$msgerror";
				}
				if($isajax!="ajax"){
	...
	function list_spp($id){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			if($id == "verifikasi"){
				$proses['Tambah SPP Baru'] = array('GET', site_url()."/home/ppomn/spp/new", '1');
			}
			$proses['Preview Data SPU'] = array('GET', site_url()."/home/ppomn/spu/preview", '1');
			$this->newtable->menu($proses);
