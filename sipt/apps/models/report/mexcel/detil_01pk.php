...
class Detil_01PK extends Model{
	function set_excel_sarana(){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			$this->newphpexcel->setActiveSheetIndex(0)->setCellValue('A1', 'LAPORAN HASIL PEMERIKSAAN SARANA PRODUKSI SUPLEMEN MAKANAN')->setCellValue('A2', 'Jenis Sarana')->setCellValue('C2', $judul)->setCellValue('A3', 'Pemeriksaan Awal')->setCellValue('C3', $awal)->setCellValue('A4', 'Pemeriksaan Akhir')->setCellValue('C4', $akhir)->setCellValue('A5', 'Balai Besar / Balai POM')->setCellValue('C5', $balai);		
			...
			if($data){
				...
				foreach($query->result_array() as $row){
					...
					->setCellValue('H'.$rec,preg_replace('/<[^>]*>/','',str_replace("&nbsp;","",html_entity_decode($row["KEPATUHAN_CPPKB"]))))
	...
	function rekap_01PK(){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			$this->newphpexcel->setActiveSheetIndex(0)->setCellValue('A6','No.')->setCellValue('B6','BB / BPOM')->setCellValue('C6','Jumlah')->setCellValue('G6','Rincian TMK (Sarana)')->setCellValue('L6','Tindak Lanjut (Sarana)')
