...
class _011M extends Model {
	...
	if ($this->newsession->userdata('LOGGED_IN') == TRUE) {
	    ...
	    $arrdata ['cb_tindakan'] = $this->config->item("tl_pusat_iklan_smpk");
	    $arrdata ['cb_tindakan2'] = $this->config->item("tl_pusat_iklan_smpk2");
	    $arrdata ['cb_tindakan_balai'] = $this->config->item("tl_balai_iklan_smpk");
	    $arrdata ['jenisProduk'] = $sipt->main->combobox("SELECT URAIAN, KODE FROM M_TABEL WHERE JENIS = 'JO_SM' ORDER BY KODE ASC", "KODE", "URAIAN", TRUE);
	...
	$arr_iklan_SM = $this->input->post('IKLANSM');
	$arr_petugas = $this->session->userdata('USER');
	$lampiran = $this->input->post('IKLAN_SM');
	...
	if ($this->db->affected_rows() > 0) {
		...
		foreach ($arr_produk['NAMA'] as $a) {
		    $produk = array('IKLAN_ID' => $iklan_id, 'IKLAN_ID_PRODUK' => $iklan_id . $i, 'NAMA_PRODUK' => $arr_produk['NAMA'][$i], 'BENTUK_SEDIAAN' => $arr_produk['BENTUKSEDIAAN'][$i], 'NAMA_PEMILIK_IZIN_EDAR' => $arr_produk['NAMAPEMILIKIZINEDAR'][$i], 'MERK_PRODUK' => $arr_produk['MERK_PRODUK'][$i], 'NOMOR_IZIN_EDAR' => $arr_produk['NOMORIZINEDAR'][$i], 'GOLONGAN_PRODUK' => $arr_produk['GOLONGAN'][$i], 'JENIS_PRODUK' => $arr_produk['JENIS'][$i]);
	...
	if ($this->db->affected_rows() > 0) {
	    $iklanSM = array('IKLAN_ID' => $iklan_id, 'URAIAN_PELANGGARAN' => $arr_uraian_pelanggaran, 'NARASI' => $arr_iklan_SM['NARASI'], 'LAMPIRAN' => $lampiran['FILE_IKLAN'], 'TL_BALAI' => $arr_iklan['TL_BALAI'], 'PEMUSNAHAN' => $pemusnahan, 'PEMUSNAHANPUSAT' => $pemusnahanPusat);
	    $this->db->insert('T_IKLAN_PK_SM', $iklanSM);
	    if ($this->db->affected_rows() > 0) {
		$case = 'YESIKLANSM';
	    } else {
		$case = 'NO';
	...
	$sipt = & get_instance();
	$this->load->model("main", "main", true);
	$query = "SELECT TIPS.URAIAN_PELANGGARAN, TI.STATUS, TI.SURAT_ID, TI.IKLAN_ID, TI.HASIL, TI.HASIL_PUSAT, TI.JUSTIFIKASI_PUSAT, CONVERT(VARCHAR(10), TI.TANGGAL_MULAI, 120) AS TANGGAL_MULAI, CONVERT(VARCHAR(10), TI.TANGGAL_AKHIR, 120) AS TANGGAL_AKHIR FROM T_IKLAN TI RIGHT JOIN T_IKLAN_PK_SM TIPS ON TIPS.IKLAN_ID = TI.IKLAN_ID WHERE TIPS.IKLAN_ID = '" . $idPengawasan . "'";
	...
	if (($subid == '0101' || $subid == '1101') && !in_array($this->newsession->userdata('SESS_BBPOM_ID'), $this->config->item('cfg_unit')) && array_key_exists('2', $this->newsession->userdata('SESS_KODE_ROLE')) && !in_array($bbpom, $this->config->item('cfg_unit')) && ($jenis == 'draft' || $jenis == 'edit')) {
	    $query2 = "SELECT TIP.NAMA_PRODUK, TIP.BENTUK_SEDIAAN, TIP.NAMA_PEMILIK_IZIN_EDAR, TIP.MERK_PRODUK, TIP.NOMOR_IZIN_EDAR, TIP.JENIS_PRODUK FROM T_IKLAN TI RIGHT JOIN T_IKLAN_PRODUK TIP ON TIP.IKLAN_ID = TI.IKLAN_ID WHERE TIP.IKLAN_ID ='" . $arrdata['sess']['IKLAN_ID'] . "'";
	    ...
	    $query2 = "SELECT TIP.NAMA_PRODUK, TIP.BENTUK_SEDIAAN, TIP.NAMA_PEMILIK_IZIN_EDAR, TIP.NOMOR_IZIN_EDAR, TIP.MERK_PRODUK, TIP.JENIS_PRODUK FROM T_IKLAN TI RIGHT JOIN T_IKLAN_PRODUK TIP ON TIP.IKLAN_ID = TI.IKLAN_ID WHERE TIP.IKLAN_ID ='" . $arrdata['sess']['IKLAN_ID'] . "'";
	    $data2 = $sipt->main->get_result($query2);
	    if ($data2) {
		foreach ($query2->result_array() as $row) {
		    $str[] = $row;
	    ...
	    $query2 = "SELECT TIP.NAMA_PRODUK, TIP.BENTUK_SEDIAAN, TIP.NAMA_PEMILIK_IZIN_EDAR, TIP.NOMOR_IZIN_EDAR, TIP.MERK_PRODUK, (SELECT URAIAN FROM M_TABEL WHERE KODE = TIP.JENIS_PRODUK AND JENIS = 'JO_SM') AS JENIS FROM T_IKLAN TI RIGHT JOIN T_IKLAN_PRODUK TIP ON TIP.IKLAN_ID = TI.IKLAN_ID WHERE TIP.IKLAN_ID ='" . $arrdata['sess']['IKLAN_ID'] . "'";
	    $premarketArr = array(0 => "Iklan sudah dipre-review", 1 => "Iklan belum dipre-review");
	    $data2 = $sipt->main->get_result($query2);
	    if ($data2) {
		$str = '<table class="tabelajax"><tr class="head"><th>Nama Produk OT</th><th>Bentuk Sediaan</th><th>Nama Pemilik Izin Edar</th><th>Nomor Izin Edar</th><th>Kategori Produk</th><th>Evaluasi Premarket</th></tr>';
		foreach ($query2->result_array() as $row) {
		    $premarket = trim($row['MERK_PRODUK']) != "" ? $premarketArr[$row['MERK_PRODUK']] : "-";
		    $str .= '<tr><td>' . $row['NAMA_PRODUK'] . '</td><td>' . $row['BENTUK_SEDIAAN'] . '</td><td>' . $row['NAMA_PEMILIK_IZIN_EDAR'] . '</td><td>' . $row['NOMOR_IZIN_EDAR'] . '</td><td>' . $row['JENIS'] . '</td><td>' . $premarket . '</td></tr>';
		}
		$str .= '</table>';
	    }
	}
	...
	$arrdata['tujuan'] = $user;
	$arrdata ['jenisProduk'] = $sipt->main->combobox("SELECT URAIAN, KODE FROM M_TABEL WHERE JENIS = 'JO_SM'", "KODE", "URAIAN", TRUE);
	...
	if ($this->newsession->userdata('LOGGED_IN') == TRUE) {
		...
		if ($arr_iklan_SM) {
		    $iklanSM = array('URAIAN_PELANGGARAN' => $arr_uraian_pelanggaran, 'NARASI' => $arr_iklan_SM['NARASI'], 'LAMPIRAN' => $lampiran['FILE_IKLAN'], 'TL_BALAI' => $arr_iklan['TL_BALAI']);
		    $this->db->where(array("IKLAN_ID" => $id));
		    $this->db->update('T_IKLAN_PK_SM', $iklanSM);
		    if ($this->db->affected_rows() > 0) {
			$case = 'YESIKLANSM';
	    ...
	    if ($case == 'YESIKLANSM' || $case == 'YESIKLAN' || $case == 'YESIKLANPRODUK') {
	...
	if ($this->newsession->userdata('LOGGED_IN') == TRUE) {
	    ...
	    $query = "SELECT DISTINCT (REPLACE(REPLACE(MB.NAMA_BBPOM,'BALAI BESAR POM DI ', ''),'BALAI POM DI ','')) AS NAMA_BBPOM,  TI.JENIS_IKLAN, (SELECT TI.MEDIA WHERE TI.JENIS_IKLAN = 'Cetak') AS CETAK, (SELECT TI.MEDIA WHERE TI.JENIS_IKLAN = 'Elektronik') AS ELEKTRO, (SELECT TI.MEDIA WHERE TI.JENIS_IKLAN = 'Luar Ruang') AS LUAR, SUM(CASE WHEN TI.HASIL = 'MK' AND TI.STATUS <> 60310 THEN 1 ELSE 0 END) AS MK_B, SUM(CASE WHEN TI.HASIL = 'TMK' AND TI.STATUS <> 60310 THEN 1 ELSE 0 END) AS TMK_B, SUM(CASE WHEN TI.HASIL_PUSAT = 'MK' AND TI.STATUS = 60310 THEN 1 ELSE 0 END) AS MK_P, SUM(CASE WHEN TI.HASIL_PUSAT = 'TMK' AND TI.STATUS = 60310 THEN 1 ELSE 0 END) AS TMK_P, SUM(CASE WHEN TIP.MERK_PRODUK = '0' THEN 1 ELSE 0 END) AS SUDAH, SUM(CASE WHEN TIP.MERK_PRODUK = '1' THEN 1 ELSE 0 END) AS BELUM, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%0#%' THEN 1 ELSE 0 END) AS UR0, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#1#%' THEN 1 ELSE 0 END) AS UR1, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#2#%' THEN 1 ELSE 0 END) AS UR2, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#3#%' THEN 1 ELSE 0 END) AS UR3, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#4#%' THEN 1 ELSE 0 END) AS UR4, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#5#%' THEN 1 ELSE 0 END) AS UR5, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#6#%' THEN 1 ELSE 0 END) AS UR6, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#7#%' THEN 1 ELSE 0 END) AS UR7, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#8#%' THEN 1 ELSE 0 END) AS UR8, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#9#%' THEN 1 ELSE 0 END) AS UR9, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#10#%' THEN 1 ELSE 0 END) AS UR10, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#11#%' THEN 1 ELSE 0 END) AS UR11, SUM(CASE WHEN TIPS.URAIAN_PELANGGARAN LIKE '%#12#%' THEN 1 ELSE 0 END) AS UR12 FROM T_IKLAN TI LEFT JOIN M_BBPOM MB ON TI.BBPOM_ID = MB.BBPOM_ID LEFT JOIN T_IKLAN_PRODUK TIP ON TIP.IKLAN_ID = TI.IKLAN_ID LEFT JOIN T_IKLAN_PK_SM TIPS ON TIPS.IKLAN_ID = TI.IKLAN_ID WHERE $filter2 AND TI.KOMODITI = '011' GROUP BY MB.BBPOM_ID, MB.NAMA_BBPOM, TI.MEDIA, TI.JENIS_IKLAN ORDER BY 1";
	...
	if ($this->newsession->userdata('LOGGED_IN') == TRUE) {
		...
		$no = 1;
		$rec = 8;
		$uraianArr = $this->config->item('uraian_iklan_smpk');
		$TLPusatArr = $this->config->item('tl_pusat_iklan_smpk2');
		$TLBalaiArr = $this->config->item('tl_balai_iklan_smpk');
		foreach ($query->result_array() as $row) {
