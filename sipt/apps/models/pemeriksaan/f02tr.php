...
class F02TR extends Model{
	
	function GetForm02TR($sarana, $jenis, $klasifikasi, $idperiksa){
...
$qsarana = "SELECT NAMA_SARANA, ALAMAT_1, ALAMAT_2, TELEPON, NAMA_PIMPINAN, PENANGGUNG_JAWAB, NOMOR_IZIN, TANGGAL_IZIN, NO_SIK FROM M_SARANA WHERE SARANA_ID='$sarana'";
				...
				if($dt_sarana){
					foreach($qsarana->result_array() as $row){
						$arrdata = array('sess' => $row,
										 ...
										 'temuan_produk' => '',
										 'sel_tujuan' => 'Rutin',
										 'act' => site_url().'/post/pemeriksaan/set_periksa/03TR/simpan',
				...
				*/
				...
				if($this->newsession->userdata('SESS_BBPOM_ID') != "92"){#Update dan Perbaikan Balai
					if($status=="20101"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/03TR/update';
					}else if($status=="20102" || $status =="20103"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/03TR/perbaikan';
					}else if($status=="60020"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/03TR/rekomendasi';
					}
	...
	function SaveForm02TR($action, $isajax){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			$sipt =& get_instance();
		...
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE ){
			...
			*/
			...
			if(array_key_exists('2', $this->newsession->userdata('SESS_KODE_ROLE'))){#Operator
				  ...
				  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02TR/operator';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02TR/spv-satu';
				...
				$arrdata['obj_status'] = 'SUPERVISOR[STATUS]';
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02TR/spv-dua';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02TR/56';
			}
			if($subid!=""){
				$isPerbaikan = FALSE;
				$isEditTLBalai = FALSE;
