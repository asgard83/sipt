...
class F02OT extends Model{
	
	function GetForm02OT($sarana, $jenis, $klasifikasi, $idperiksa){
				  ...
				  if($dt_sarana){
					  foreach($qsarana->result_array() as $row){
						  $arrdata = array('sess' => $row,							
										   ...
										   'act' => site_url().'/post/pemeriksaan/set_periksa/02OT/simpan',
						  				   'temuan_produk' => array(),
										   'aspek_check' => '',
										   'aspek_keterangan' => '',
										   'sel_tl_balai' => '',
...
C.ASPEK_CHECK, C.ASPEK_KETERANGAN, C.HASIL_TEMUAN_LAIN, C.PEMANTAUAN_HASIL, B.HASIL, C.CATATAN, C.DETIL_HASIL, C.KESIMPULAN_DETIL_TMK, C.TINDAK_LANJUT_BALAI, C.TINDAK_LANJUT_PUSAT, C.SARAN_TL, D.NAMA_PRODUK, D.KLASIFIKASI_PRODUK, D.NETTO, D.NOMOR_REGISTRASI, D.SATUAN, D.NO_BATCH, D.TANGGAL_EXPIRE, D.NAMA_PERUSAHAAN, D.ALAMAT_PERUSAHAAN, D.KATEGORI, D.JUMLAH_TEMUAN, D.HARGA_SATUAN, D.HARGA_TOTAL, D.TINDAKAN_PRODUK, D.KETERANGAN_SUMBER, D.JENIS_PELANGGARAN, D.TINDAKAN_PRODUK, STUFF(dbo.GROUP_CONCAT(B.PERIKSA_ID),1,1,'') AS SURAT_ID FROM M_SARANA A LEFT JOIN T_PEMERIKSAAN B ON A.SARANA_ID = B.SARANA_ID LEFT JOIN T_PEMERIKSAAN_OT C ON B.PERIKSA_ID = C.PERIKSA_ID LEFT JOIN T_PEMERIKSAAN_TEMUAN_PRODUK D ON B.PERIKSA_ID = D.PERIKSA_ID WHERE B.SARANA_ID = '$sarana' AND B.PERIKSA_ID = '$idperiksa[0]'";
				...
				if($this->newsession->userdata('SESS_BBPOM_ID') != "94"){#Update dan Perbaikan Balai
					if($status=="20101"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/02OT/update';
					}else if($status=="20102" || $status =="20103"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/02OT/perbaikan';
					}else if($status =="60020"){
						  $arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/02OT/rekomendasi';
					}
	...
	function SaveForm02OT($action, $isajax){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			$sipt =& get_instance();
			...
			if($action=="simpan"){#Insert Mode
				...
				if($this->db->insert('T_PEMERIKSAAN', $arr_pemeriksaan)){
					...
					foreach($this->input->post('PEMERIKSAAN_DIST_OT') as $c => $d){
						if(!is_array($d))
							$arr_distribusi[$c] = $d;
						else
							$arr_distribusi[$c] = join("#", $d);
					}
					$this->db->insert('T_PEMERIKSAAN_OT', $arr_distribusi);
				...
				if($this->db->update('T_PEMERIKSAAN', $arr_pemeriksaan)){	
					...
					$this->db->where(array("PERIKSA_ID" => $this->input->post('PERIKSA_ID')));
					$this->db->update('T_PEMERIKSAAN_OT', $arr_distribusi);
	...
	function input_preview($sarana,$jenis,$idperiksa,$subid){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE ){
			...
			$stat = $idperiksa[1];
			$judul = $sipt->main->get_judul($jenis);
			$query = "SELECT (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_TEMUAN_PRODUK WHERE PERIKSA_ID='$idperiksa[0]') AS JMLTEMUAN, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_PROSES WHERE PERIKSA_ID='$idperiksa[0]') AS JML_PROSES, A.SARANA_ID,  A.NAMA_SARANA, A.ALAMAT_1, A.ALAMAT_2, A.TELEPON, A.NAMA_PIMPINAN, A.PENANGGUNG_JAWAB, B.PERIKSA_ID, B.JENIS_SARANA_ID, B.BBPOM_ID, STUFF(dbo.GROUP_KLASIFIKASI(B.PERIKSA_ID),1,1,'') AS KK_ID, CONVERT(VARCHAR(10), B.AWAL_PERIKSA, 103) AS AWAL_PERIKSA, CONVERT(VARCHAR(10), B.AKHIR_PERIKSA, 103) AS AKHIR_PERIKSA, C.TUJUAN_PEMERIKSAAN, C.KLASIFIKASI_PEMERIKSAAN, C.ASPEK_CHECK, C.ASPEK_KETERANGAN, C.HASIL_TEMUAN_LAIN, C.PEMANTAUAN_HASIL, B.HASIL, C.CATATAN, C.DETIL_HASIL, C.KESIMPULAN_DETIL_TMK, C.TINDAK_LANJUT_BALAI, C.TINDAK_LANJUT_PUSAT, C.SARAN_TL, C.CATATAN_PUSAT, C.HASIL_PUSAT, D.NAMA_PRODUK, D.KLASIFIKASI_PRODUK, D.NETTO, D.NOMOR_REGISTRASI, D.SATUAN, D.NO_BATCH, D.TANGGAL_EXPIRE, D.NAMA_PERUSAHAAN, D.ALAMAT_PERUSAHAAN, D.KATEGORI, D.JUMLAH_TEMUAN, D.HARGA_SATUAN, D.HARGA_TOTAL, D.TINDAKAN_PRODUK, D.KETERANGAN_SUMBER, D.JENIS_PELANGGARAN, D.TINDAKAN_PRODUK, STUFF(dbo.GROUP_CONCAT(B.PERIKSA_ID),1,1,'') AS SURAT_ID FROM M_SARANA A LEFT JOIN T_PEMERIKSAAN B ON A.SARANA_ID = B.SARANA_ID LEFT JOIN T_PEMERIKSAAN_OT C ON B.PERIKSA_ID = C.PERIKSA_ID LEFT JOIN T_PEMERIKSAAN_TEMUAN_PRODUK D ON B.PERIKSA_ID = D.PERIKSA_ID WHERE B.SARANA_ID = '$sarana' AND B.PERIKSA_ID = '$idperiksa[0]'";
			$data = $sipt->main->get_result($query);
			if($data){
					foreach($query->result_array() as $row){
						$temuan_produk[] = $row;
			  ...
			  if(array_key_exists('2', $this->newsession->userdata('SESS_KODE_ROLE'))){#Operator
				  if($this->newsession->userdata('SESS_BBPOM_ID') != "94"){
					  $arrdata['obj_ot'] = 'PEMERIKSAAN_DIST_OT';
				  }else{
					   $arrdata['obj_ot'] = 'PEMERIKSAAN_DIST_OT_PUSAT';
				  }				  
				  $isEditTL = TRUE;
				  $isEditTLBalai = TRUE;
				  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02OT/operator';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02OT/spv-satu';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02OT/spv-dua';
				$arrdata['obj_status'] = 'SUPERVISOR[STATUS]';
			  ...
			  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02OT/56';
			}
		...
		if($this->newsession->userdata('LOGGED_IN') == TRUE){
			...
			if($role=="operator"){
				...
				if($this->db->update('T_PEMERIKSAAN', $arr_status)){#Update Status Operator
					  if($this->newsession->userdata('SESS_BBPOM_ID') == "94"){
						  if($this->input->post('PEMERIKSAAN_DIST_OT_PUSAT')){
							  $arr_input = $this->input->post('PEMERIKSAAN_DIST_OT_PUSAT'); 
							  ...
							  $this->db->where(array("PERIKSA_ID" => $this->input->post('PERIKSA_ID')));
							  $this->db->update('T_PEMERIKSAAN_OT', $arr_update); #Update Tindak Lanjut Pusat
						  }
					  }
				...
				if($this->db->update('T_PEMERIKSAAN', $arr_status)){
					 if($this->newsession->userdata('SESS_BBPOM_ID') == "94"){
						 ...
						 if($this->input->post('PEMERIKSAAN_DIST_OT')){
						 $arr_input = $this->input->post('PEMERIKSAAN_DIST_OT'); 
						 foreach($arr_input as $c => $d){
							if(!is_array($d))
								$arr_update[$c] = $d;
							else
						 ...
						 $this->db->where(array("PERIKSA_ID" => $this->input->post('PERIKSA_ID')));
						 $this->db->update('T_PEMERIKSAAN_OT', $arr_update); #Update Tindak Lanjut
						 }
					 }
