...
class F03RS extends Model{
	
	function GetForm03RS($sarana, $jenis, $klasifikasi, $idperiksa){
...
$qsarana = "SELECT NAMA_SARANA, ALAMAT_1, TELEPON, NOMOR_IZIN, TANGGAL_IZIN, NO_SIK, NAMA_PIMPINAN, PENANGGUNG_JAWAB FROM M_SARANA WHERE SARANA_ID='$sarana'";
				...
				if($dt_sarana){
					foreach($qsarana->result_array() as $row){
						$arrdata = array('sess' => $row,
										 ...
										 'temuan_produk' => '',
										 'sel_tujuan' => 'Rutin',
										 'act' => site_url().'/post/pemeriksaan/set_periksa/03RS/simpan',
				...
				*/
				...
				if($dt_periksa){
					...
					$arrdata['url_detil_perbaikan'] = site_url().'/pemeriksaan/perbaikan/detail/'.$row['PERIKSA_ID'].'/'.$row['JENIS_SARANA_ID'];
				}
				if($this->newsession->userdata('SESS_BBPOM_ID') != "92"){#Update dan Perbaikan Balai
					if($status=="20101"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/03RS/update';
					}else if($status=="20102" || $status =="20103"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/03RS/perbaikan';
					}
	...
	function SaveForm03RS($action, $isajax){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			$sipt =& get_instance();
			...
			if($action=="simpan"){#Insert Mode
				...
				return $ret;
			}else if($action=="update" || $action == "perbaikan"){#Update
	...
	function input_preview($sarana,$jenis,$idperiksa,$subid){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE ){
			...
			*End Redirect
			*/
			$query = "SELECT (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_DISTRIBUSI_PERBAIKAN WHERE PERIKSA_ID='$idperiksa[0]') AS JML_PERBAIKAN, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_PROSES WHERE PERIKSA_ID = '$idperiksa[0]') AS JML_PROSES, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_TEMUAN_PRODUK WHERE PERIKSA_ID='$idperiksa[0]') AS JUMLAH_TEMUAN, A.SARANA_ID, A.NAMA_SARANA, A.ALAMAT_1, A.TELEPON, A.NOMOR_IZIN, A.TANGGAL_IZIN, A.NO_SIK, A.NAMA_PIMPINAN, A.PENANGGUNG_JAWAB, B.PERIKSA_ID, B.JENIS_SARANA_ID, B.BBPOM_ID,STUFF(dbo.GROUP_KLASIFIKASI(B.PERIKSA_ID),1,1,'') AS KK_ID, CONVERT(VARCHAR(10), B.AWAL_PERIKSA, 103) AS AWAL_PERIKSA, CONVERT(VARCHAR(10), B.AKHIR_PERIKSA, 103) AS AKHIR_PERIKSA, C.TUJUAN_PEMERIKSAAN, C.ASPEK_PENILAIAN, C.HASIL_TEMUAN, C.HASIL_TEMUAN_LAIN, C.CATATAN_HASIL_PEMERIKSAAN, B.HASIL, C.KASUS_POINT_A, C.KASUS_POINT_B, C.KASUS_POINT_D, C.KASUS_POINT_E, C.KASUS_POINT_F, C.KASUS_POINT_G, C.KASUS_POINT_H, C.KLASIFIKASI_PELANGGARAN_MAJOR AS MAJOR, C.KLASIFIKASI_PELANGGARAN_MINOR AS MINOR, C.KLASIFIKASI_PELANGGARAN_CRITICAL AS CRITICAL, KLASIFIKASI_PELANGGARAN_CRITICAL_ABSOLUTE AS CRITICAL_ABSOLUT, C.TINDAK_LANJUT_BALAI, C.DETAIL_TINDAK_LANJUT_BALAI, C.TINDAK_LANJUT_PUSAT, C.DETIL_TINDAK_LANJUT_PUSAT, STUFF(dbo.GROUP_CONCAT(B.PERIKSA_ID),1,1,'') AS SURAT_ID, E.KATEGORI, E.NAMA_PRODUK, E.NAMA_PABRIK, E.NEGARA_ASAL, E.KEMASAN, E.NOMOR_REGISTRASI, E.NO_BATCH, E.TANGGAL_EXPIRE, E.JUMLAH_TEMUAN, E.SATUAN, E.NAMA_PERUSAHAAN, E.ALAMAT_PERUSAHAAN, E.PEMILIK, E.TINDAKAN_PRODUK, E.KETERANGAN_SUMBER, D.URAIAN AS UR_HASIL FROM M_SARANA A LEFT JOIN T_PEMERIKSAAN B ON A.SARANA_ID = B.SARANA_ID LEFT JOIN T_PEMERIKSAAN_DISTRIBUSI C ON B.PERIKSA_ID = C.PERIKSA_ID LEFT JOIN T_PEMERIKSAAN_TEMUAN_PRODUK E ON B.PERIKSA_ID = E.PERIKSA_ID LEFT JOIN M_TABEL D ON B.HASIL = D.KODE WHERE B.SARANA_ID='$sarana' AND B.PERIKSA_ID='$idperiksa[0]' AND D.JENIS='HASIL'";
			$data = $sipt->main->get_result($query);
			if($data){
				foreach($query->result_array() as $row){
					$temuan_produk[] = $row;
			...
			if(array_key_exists('2', $this->newsession->userdata('SESS_KODE_ROLE'))){#Operator
				  ...
				  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03RS/operator';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03RS/spv-satu';
				...
				$arrdata['obj_status'] = 'SUPERVISOR[STATUS]';
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03RS/spv-dua';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03RS/56';
			}
			if($subid!=""){
				$isPerbaikan = FALSE;
				$isEditTLBalai = FALSE;
			...
			if($role=="operator"){
				...
				if($this->db->update('T_PEMERIKSAAN', $arr_status)){
					  if($this->newsession->userdata('SESS_BBPOM_ID') == "92")
						  ...
						  $arr_input = $this->input->post('PEMERIKSAAN_DISTRIBUSI'); 
					  foreach($arr_input as $c => $d){
						  if(!is_array($d))
							  $arr_update[$c] = $d;
						  else
					  ...
					  $this->db->where(array("PERIKSA_ID" => $this->input->post('PERIKSA_ID')));
					  $this->db->update('T_PEMERIKSAAN_DISTRIBUSI', $arr_update); #Update Tindak Lanjut
