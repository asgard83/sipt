...
class F03AA extends Model{
	
	function GetForm03AA($sarana, $jenis, $klasifikasi, $idperiksa){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			if($idperiksa==""){#Input Mode				  
				...
				$qsarana = "SELECT NAMA_SARANA, ALAMAT_1, TELEPON, NOMOR_IZIN, TANGGAL_IZIN, NO_SIK, NAMA_APA, NAMA_PSA, NAMA_APOTEKER_PENDAMPING FROM M_SARANA WHERE SARANA_ID='$sarana'";
				...
				if($dt_sarana){
					foreach($qsarana->result_array() as $row){
						$arrdata = array('sess' => $row,
										 ...
										 'temuan_produk' => '',
										 'sel_tujuan'=> 'Rutin',
										 'act' => site_url().'/post/pemeriksaan/set_periksa/03AA/simpan',
				...
				*/
				...
				if($this->newsession->userdata('SESS_BBPOM_ID') != "92"){#Update dan Perbaikan Balai
					if($status=="20101"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/03AA/update';
					}else if($status=="20102" || $status =="20103"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/03AA/perbaikan';
					}else if($status=="60020"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/03AA/rekomendasi';
					}
	...
	function SaveForm03AA($action, $isajax){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			$sipt =& get_instance();
		...
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE ){
			...
			*/
			...
			if(array_key_exists('2', $this->newsession->userdata('SESS_KODE_ROLE'))){#Operator
				  ...
				  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03AA/operator';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03AA/spv-satu';
				...
				$arrdata['obj_status'] = 'SUPERVISOR[STATUS]';
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03AA/spv-dua';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03AA/56';
			}
			if($subid!=""){
				$isPerbaikan = FALSE;
				$isEditTLBalai = FALSE;
