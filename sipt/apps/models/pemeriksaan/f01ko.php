...
class F01KO extends Model{
	
	function GetForm01KO($sarana, $jenis, $klasifikasi, $idperiksa){
...
$qsarana = "SELECT(SELECT COUNT(SARANA_ID) FROM T_PEMERIKSAAN WHERE SARANA_ID='$sarana' AND STATUS NOT IN('20')) AS PERIKSA_SEBELUMNYA, A.SARANA_ID, A.NAMA_SARANA, A.ALAMAT_1, A.ALAMAT_2, A.JENIS_INDUSTRI, A.NAMA_PIMPINAN, A.PENANGGUNG_JAWAB, A.KEGIATAN_SARANA FROM M_SARANA A LEFT JOIN T_PEMERIKSAAN B ON A.SARANA_ID = B.SARANA_ID WHERE A.SARANA_ID ='$sarana'";
				  ...
				  if($dt_sarana){
					  foreach($qsarana->result_array() as $row){
						  $arrdata = array('sess' => $row,
										   ...
										   'act' => site_url().'/post/pemeriksaan/set_periksa/01KO/simpan',
										   'temuan_cpkb' => array(),
										   'sel_hasil' => 'MK',
										   'sel_tmk' => '',
										   'detil_kesimpulan_tmk' => '',
										   'sel_tindakan_observasi' => '',
										   'temuan_produk' => array(),
				...
				$status = $idperiksa[1];
				$qperiksa = "SELECT(SELECT COUNT(SARANA_ID) FROM T_PEMERIKSAAN WHERE SARANA_ID = '$sarana' AND STATUS IN('20')) AS PERIKSA_SEBELUMNYA, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_TEMUAN_PRODUK WHERE PERIKSA_ID='$idperiksa[0]') AS JUMLAH_TEMUAN, A.SARANA_ID, A.NAMA_SARANA, A.ALAMAT_1, A.ALAMAT_2, A.JENIS_INDUSTRI, A.NAMA_PIMPINAN, A.PENANGGUNG_JAWAB, A.KEGIATAN_SARANA, B.PERIKSA_ID, B.JENIS_SARANA_ID, B.BBPOM_ID, STUFF(dbo.GROUP_KLASIFIKASI(B.PERIKSA_ID),1,1,'') AS KK_ID, CONVERT(VARCHAR(10), B.AWAL_PERIKSA, 103) AS AWAL_PERIKSA, CONVERT(VARCHAR(10), B.AKHIR_PERIKSA, 103) AS AKHIR_PERIKSA, C.TUJUAN_PEMERIKSAAN, C.KEPATUHAN_CPKB, C.LATAR_BELAKANG, C.PERUBAHAN_BERMAKNA, C.RUANG_LINGKUP, C.AREA_INSPEKSI, C.DISTRIBUSI_PENGANGKUTAN, C.PERMOHONAN_PENDAFTARAN_PRODUK, C.ISU_SPESIFIK_LAINNYA, C.SITE_FILE_MASTER, C.LAIN_LAIN, C.SAMPEL_DIAMBIL, C.DISTRIBUSI_LAPORAN, C.LAMPIRAN, C.KESIMPULAN_OBSERVASI, C.TINDAKAN_OBSERVASI, C.DIP_A, C.DIP_B, C.DIP_C, C.DIP_D, C.KESIMPULAN_DIP, C.TINDAK_LANJUT_DIP,C.TEMUAN_KRITIKAL, C.TEMUAN_MAJOR, C.TEMUAN_MINOR, B.HASIL, C.DETIL_HASIL, C.KESIMPULAN_DETIL_TMK, C.REKOMENDASI, C.KESIMPULAN, C.TINDAK_LANJUT, C.TIME_LINE, C.STATUS_CAPA, C.CAPA_CLOSED, D.NAMA_PRODUK, D.KLASIFIKASI_PRODUK, D.NETTO, D.NOMOR_REGISTRASI, D.SATUAN, D.NO_BATCH, D.TANGGAL_EXPIRE, D.NAMA_PRODUK, D.KLASIFIKASI_PRODUK, D.NOMOR_REGISTRASI, D.NO_BATCH, D.NETTO, D.TANGGAL_EXPIRE, D.NAMA_PERUSAHAAN, D.ALAMAT_PERUSAHAAN, D.JENIS_PELANGGARAN, D.KATEGORI, D.JUMLAH_TEMUAN, D.HARGA_SATUAN, D.SATUAN, D.HARGA_TOTAL, D.TINDAKAN_PRODUK, D.KETERANGAN_SUMBER, STUFF(dbo.GROUP_CONCAT(B.PERIKSA_ID),1,1,'') AS SURAT_ID FROM M_SARANA A LEFT JOIN T_PEMERIKSAAN B ON A.SARANA_ID = B.SARANA_ID LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPKB C ON B.PERIKSA_ID = C.PERIKSA_ID LEFT JOIN T_PEMERIKSAAN_TEMUAN_PRODUK D ON B.PERIKSA_ID = D.PERIKSA_ID WHERE B.SARANA_ID = '$sarana' AND B.PERIKSA_ID = '$idperiksa[0]'";
				...
				if($dt_periksa){
				  ...
				  if($this->newsession->userdata('SESS_BBPOM_ID') != "94"){#Update dan Perbaikan Balai
					  if($status=="20101"){
						  $arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/01KO/update';
					  }else if($status=="20102" || $status =="20103"){
						  $arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/01KO/perbaikan';
					  }else if($status =="60020"){
						  $arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/01KO/rekomendasi';
					  }
				  ...
				  $arrdata['temuan_cpkb'] = $sipt->main->get_observasi($idperiksa[0], "KOS", "");
			...
			if($this->newsession->userdata('SESS_BBPOM_ID') == "94")
				$arrdata['tujuan_pemeriksaan'] = $sipt->main->referensi("TUJUAN_PEMERIKSAAN","'16','1','2','3','13','15'",TRUE);
			else
				$arrdata['tujuan_pemeriksaan'] = $sipt->main->referensi("TUJUAN_PEMERIKSAAN","'16','1','2'",TRUE);
			$arrdata['tindakan_observasi'] = $sipt->main->referensi("TL_OBSERVASI","'01','08','09'",TRUE);
			...
			$arrdata['tindakan_produk'] = $sipt->main->referensi("TL_PRODUK_TEMUAN","'01','02','03','04','05'",TRUE,TRUE);
			$arrdata['kesimpulan_observasi'] = array("Menerapkan" => "Menerapkan", "Tidak Menerapkan" => "Tidak Menerapkan");
			$arrdata['kesimpulan_dip'] = array("Ada, Lengkap" => "Ada, Lengkap", "Ada, Tidak Lengkap" => "Ada, Tidak Lengkap", "Tidak Ada" => "Tidak Ada");
			$arrdata['detil_tmk'] = $sipt->main->referensi("DTL_TMK","'05','06','07','08','09','10'",TRUE);
			$arrdata['cb_observasi'] = $sipt->main->referensi("CB_OBSERVASI","",TRUE,TRUE);
			$arrdata['temuan_observasi'] = $sipt->main->referensi("TEMUAN_OBSERVASI_CPKP","",FALSE,TRUE);
			$arrdata['tl_cpkb'] = $sipt->main->referensi("TL_CPKB","",TRUE,TRUE);
			$arrdata['capa_cpkb'] = $sipt->main->referensi("CAPA_OTSM","",TRUE,TRUE);
			$arrdata['capa_close_cpkb'] = $sipt->main->referensi("CLOSED_CPKB","",TRUE,TRUE);
	...
	function get_temuan($sarana, $periksa, $isprev){
		...
		$temuan_observasi = $sipt->main->combobox("SELECT KODE, URAIAN FROM M_TABEL WHERE JENIS='TEMUAN_OBSERVASI_CPKP' ORDER BY KODE ASC","KODE","URAIAN", TRUE);
		if($periksa==""){
			$arrdata = array('sess_temuan' => '', 'isprev' => $isprev);	
		}else{
			$query = "SELECT (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_PRODUKSI_CPKB_TEMUAN WHERE PERIKSA_ID='$periksa') AS JUMLAH_TEMUAN, A.SARANA_ID, C.SERI, C.TEMUAN_OBSERVASI, C.TEMUAN_TEKS, C.TEMUAN_KRITERIA, C.TEMUAN_FILE, D.URAIAN AS UR_JENIS FROM T_PEMERIKSAAN A LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPKB B ON A.PERIKSA_ID = B.PERIKSA_ID LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPKB_TEMUAN C ON B.PERIKSA_ID = C.PERIKSA_ID LEFT JOIN M_TABEL D ON D.KODE = C.TEMUAN_OBSERVASI WHERE A.PERIKSA_ID = '$periksa' AND A.SARANA_ID = '$sarana' AND D.JENIS='TEMUAN_OBSERVASI_CPKP'";
			$data = $sipt->main->get_result($query);
			if($data){
				foreach($query->result_array() as $row){
					$temuan_cpkb[] = $row;
					$arrdata = array('sess_temuan' => $row, 'temuan_cpkb' => $temuan_cpkb, 'isprev' => $isprev);
				}
			}
		}
		$arrdata['sarana_id'] = $sarana;
	...
	function SaveForm01KO($action, $isajax){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			$sipt =& get_instance();
			$this->load->model("main","main", true);
			...
			if($action=="simpan"){#Insert
				...
				if($this->db->insert('T_PEMERIKSAAN', $arr_pemeriksaan)){
					...
					$arr_cpkb = array('PERIKSA_ID' => $periksa_id);
					foreach($this->input->post('PEMERIKSAAN_CPKB') as $c => $d){
						if(!is_array($d))
							$arr_cpkb[$c] = $d;
						else
							$arr_cpkb[$c] = join("#", $d);
					}
					$this->db->insert('T_PEMERIKSAAN_PRODUKSI_CPKB', $arr_cpkb);
					...
					if($this->input->post('TEMUAN')){
						...
						for($i=0;$i<count($arrtemuan[$arrkeys[0]]);$i++){
							$seri = (int)$sipt->main->get_uraian("SELECT MAX(SERI) AS MAXID FROM T_PEMERIKSAAN_PRODUKSI_CPKB_TEMUAN WHERE PERIKSA_ID = '$periksa_id'", "MAXID") + 1;
							...
							$this->db->insert('T_PEMERIKSAAN_PRODUKSI_CPKB_TEMUAN', $temuan);
						}
					}
				...
				if($this->db->update('T_PEMERIKSAAN', $arr_pemeriksaan)){
					...
					$this->db->where(array("PERIKSA_ID" => $this->input->post('PERIKSA_ID')));
					$this->db->update('T_PEMERIKSAAN_PRODUKSI_CPKB', $arr_cpkb);
				}
				
				if($this->input->post('TEMUAN')){
					  $this->db->where('PERIKSA_ID', $this->input->post('PERIKSA_ID'));
					  $this->db->delete('T_PEMERIKSAAN_PRODUKSI_CPKB_TEMUAN');
					  $arrtemuan = $this->input->post('TEMUAN');
					  $arrkeys = array_keys($arrtemuan);
					  for($i=0;$i<count($arrtemuan[$arrkeys[0]]);$i++){
						  $seri = (int)$sipt->main->get_uraian("SELECT MAX(SERI) AS MAXID FROM T_PEMERIKSAAN_PRODUKSI_CPKB_TEMUAN WHERE PERIKSA_ID = '".$this->input->post('PERIKSA_ID')."'", "MAXID") + 1;
	...
	function get_preview($sarana,$id,$jenis){
		...
		$this->load->model("main","main", true);
		$id = explode(".", $id);
		$query = "SELECT (CAST(A.SARANA_ID AS VARCHAR) + '/' + A.JENIS_SARANA_ID + '/' + STUFF(dbo.GROUP_KLASIFIKASI(A.PERIKSA_ID),1,1,'') + '/' + CAST(A.PERIKSA_ID AS VARCHAR) + '.' + A.STATUS) AS IDPERIKSA, A.PERIKSA_ID, A.SARANA_ID, A.STATUS, CONVERT(VARCHAR(10), A.AWAL_PERIKSA, 103) AS [AWAL_PERIKSA], CONVERT(VARCHAR(10), A.AKHIR_PERIKSA, 103) AS [AKHIR_PERIKSA], B.KEPATUHAN_CPKB, B.LATAR_BELAKANG, B.PERUBAHAN_BERMAKNA, B.RUANG_LINGKUP, B.AREA_INSPEKSI, B.TEMUAN_KRITIKAL, B.TEMUAN_MAJOR, B.TEMUAN_MINOR, A.HASIL, B.DETIL_HASIL, B.KESIMPULAN_DETIL_TMK, B.REKOMENDASI, B.KESIMPULAN, STUFF(dbo.GROUP_CONCAT(A.PERIKSA_ID),1,1,'') AS SURAT_ID FROM T_PEMERIKSAAN A LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPKB B ON A.PERIKSA_ID = B.PERIKSA_ID WHERE A.SARANA_ID = '$sarana' AND A.PERIKSA_ID = '$id[0]'";
		$judul = $sipt->main->get_judul($jenis);
		$data = $sipt->main->get_result($query);
		$observasi = $sipt->main->get_observasi($id[0], "KOS", "");
		if($data){
			foreach($query->result_array() as $row){
	...
	function input_preview($sarana,$jenis,$idperiksa,$subid){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE ){
			...
			$idperiksa = explode(".", $idperiksa);
			$stat = $idperiksa[1];
			$query = "SELECT(SELECT COUNT(SARANA_ID) FROM T_PEMERIKSAAN WHERE SARANA_ID = '".$sarana."' AND STATUS IN('20')) AS PERIKSA_SEBELUMNYA, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_TEMUAN_PRODUK WHERE PERIKSA_ID='".$idperiksa[0]."') AS JMLTEMUAN, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_PRODUKSI_CPOB_PERBAIKAN WHERE PERIKSA_ID='".$idperiksa[0]."') AS JML_PERBAIKAN, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_PROSES WHERE PERIKSA_ID = '".$idperiksa[0]."') AS JML_PROSES, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_PRODUKSI_CPKB_PERBAIKAN WHERE PERIKSA_ID='".$idperiksa[0]."') AS JML_PERBAIKAN, A.SARANA_ID, A.NAMA_SARANA, A.ALAMAT_1, A.ALAMAT_2, A.JENIS_INDUSTRI, A.NAMA_PIMPINAN, A.PENANGGUNG_JAWAB, A.KEGIATAN_SARANA, B.PERIKSA_ID, B.JENIS_SARANA_ID, B.BBPOM_ID, STUFF(dbo.GROUP_KLASIFIKASI(B.PERIKSA_ID),1,1,'') AS KK_ID, CONVERT(VARCHAR(10), B.AWAL_PERIKSA, 103) AS AWAL_PERIKSA, CONVERT(VARCHAR(10), B.AKHIR_PERIKSA, 103) AS AKHIR_PERIKSA, C.TUJUAN_PEMERIKSAAN, C.KEPATUHAN_CPKB, C.LATAR_BELAKANG, C.PERUBAHAN_BERMAKNA, C.RUANG_LINGKUP, C.AREA_INSPEKSI, C.DISTRIBUSI_PENGANGKUTAN, C.PERMOHONAN_PENDAFTARAN_PRODUK, C.ISU_SPESIFIK_LAINNYA, C.SITE_FILE_MASTER, C.LAIN_LAIN, C.SAMPEL_DIAMBIL, C.DISTRIBUSI_LAPORAN, C.LAMPIRAN, C.KESIMPULAN_OBSERVASI, C.TINDAKAN_OBSERVASI, C.DIP_A, C.DIP_B, C.DIP_C, C.DIP_D, C.KESIMPULAN_DIP, C.TINDAK_LANJUT_DIP,C.TEMUAN_KRITIKAL, C.TEMUAN_MAJOR, C.TEMUAN_MINOR, B.HASIL, C.DETIL_HASIL, C.KESIMPULAN_DETIL_TMK, C.REKOMENDASI, C.KESIMPULAN, C.TINDAK_LANJUT, C.TIME_LINE, C.STATUS_CAPA, C.CAPA_CLOSED, C.HASIL_PUSAT, C.CATATAN_PUSAT, D.NAMA_PRODUK, D.KLASIFIKASI_PRODUK, D.NETTO, D.NOMOR_REGISTRASI, D.SATUAN, D.NO_BATCH, D.TANGGAL_EXPIRE, D.NAMA_PRODUK, D.KLASIFIKASI_PRODUK, D.NOMOR_REGISTRASI, D.NO_BATCH, D.NETTO, D.TANGGAL_EXPIRE, D.NAMA_PERUSAHAAN, D.ALAMAT_PERUSAHAAN, D.JENIS_PELANGGARAN, D.KATEGORI, D.JUMLAH_TEMUAN, D.HARGA_SATUAN, D.HARGA_TOTAL, D.TINDAKAN_PRODUK, D.KETERANGAN_SUMBER, STUFF(dbo.GROUP_CONCAT(B.PERIKSA_ID),1,1,'') AS SURAT_ID FROM M_SARANA A LEFT JOIN T_PEMERIKSAAN B ON A.SARANA_ID = B.SARANA_ID LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPKB C ON B.PERIKSA_ID = C.PERIKSA_ID LEFT JOIN T_PEMERIKSAAN_TEMUAN_PRODUK D ON B.PERIKSA_ID = D.PERIKSA_ID WHERE B.SARANA_ID = '".$sarana."' AND B.PERIKSA_ID = '".$idperiksa[0]."'";
			$data = $sipt->main->get_result($query);
			if($data){
				foreach($query->result_array() as $row){
					$temuan_produk[] = $row;
			...
			if(array_key_exists('2', $this->newsession->userdata('SESS_KODE_ROLE'))){#Operator
				if($this->newsession->userdata('SESS_BBPOM_ID') != "94"){
					$isEditTL = TRUE;
					$arrdata['obj_ot'] = 'PEMERIKSAAN_CPKB';
				...
				$isEditTLBalai = TRUE;
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/01KO/operator';
			  ...
			  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/01KO/spv-satu';
			  ...
			  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/01KO/spv-dua';
			  $arrdata['obj_status'] = 'SUPERVISOR[STATUS]';
		...
		if($this->newsession->userdata('LOGGED_IN') == TRUE){
			...
			if($role=="operator"){
				...
				if($this->db->update('T_PEMERIKSAAN', $arr_status)){#Update Status Operator
					  if($this->input->post('PEMERIKSAAN_CPKB')){
						  foreach($this->input->post('PEMERIKSAAN_CPKB') as $c => $d){
							  if(!is_array($d))
								$arr_update[$c] = $d;
							  else
						  ...
						  $this->db->where(array("PERIKSA_ID" => $this->input->post('PERIKSA_ID')));
						  $this->db->update('T_PEMERIKSAAN_PRODUKSI_CPKB', $arr_update); #Update Tindak Lanjut Pusat
					  }
					  ...
					  if($this->input->post('PERBAIKAN')){
						  $perbaikan = (int)$sipt->main->get_uraian("SELECT MAX(SERI) AS MAXID FROM T_PEMERIKSAAN_PRODUKSI_CPKB_PERBAIKAN WHERE PERIKSA_ID = '".$this->input->post('PERIKSA_ID')."'", "MAXID") + 1;
						  ...
						  if(trim($arr_perbaikan['TANGGAL_PERBAIKAN']) != "" || trim($arr_perbaikan['DETAIL_PERBAIKAN']) != ""){
							  $this->db->insert('T_PEMERIKSAAN_PRODUKSI_CPKB_PERBAIKAN', $arr_perbaikan);#Perbaikan Pemeriksaan					  
						  }
					  }
				...
				if($this->db->update('T_PEMERIKSAAN', $arr_status)){
					  $arr_input = $this->input->post('PEMERIKSAAN_CPKB'); 
					  ...
					  $this->db->where(array("PERIKSA_ID" => $this->input->post('PERIKSA_ID')));
					  $this->db->update('T_PEMERIKSAAN_PRODUKSI_CPKB', $arr_update); #Update Tindak Lanjut
	...
	function get_perbaikan($periksa_id){
		$query = "SELECT A.SARANA_ID, B.SERI AS NO, CONVERT(VARCHAR(10), B.TANGGAL_PERBAIKAN, 103) AS [TANGGAL PERBAIKAN], B.DETAIL_PERBAIKAN AS [DETAIL PERBAIKAN], '<a class=\"normal\" href=\"".base_url()."files/' + CAST(A.SARANA_ID AS VARCHAR)+ '/'+ B.FILE_PERBAIKAN +'\" target=\"_blank\">View</a>' AS [FILE PERBAIKAN], C.NAMA_BBPOM AS [BALAI ATAU UNIT], D.NAMA_USER AS [NAMA PETUGAS] FROM T_PEMERIKSAAN A LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPKB_PERBAIKAN B ON A.PERIKSA_ID = B.PERIKSA_ID LEFT JOIN T_USER D ON B.CREATE_BY = D.USER_ID LEFT JOIN M_BBPOM C ON D.BBPOM_ID = C.BBPOM_ID WHERE B.PERIKSA_ID='$periksa_id'";
		$this->load->library('newtable');
		$this->newtable->search(array(array('', '')));
	...
	function set_bap($sarana,$jenis,$idperiksa,$subid){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			$idperiksa = explode(".", $idperiksa);
			$query = "SELECT(SELECT TOP 1 CONVERT(VARCHAR(10), AWAL_PERIKSA, 103) + ' s.d ' + CONVERT(VARCHAR(10), AKHIR_PERIKSA, 103) FROM T_PEMERIKSAAN WHERE SARANA_ID = '$sarana' ) AS TERAKHIR, (SELECT STUFF(dbo.GROUP_PETUGAS('$sarana'),1,0,'')) AS INSPEKTUR, (SELECT TOP 1 LATAR_BELAKANG FROM T_PEMERIKSAAN WHERE SARANA_ID = '$sarana') AS RINGKASAN, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_TEMUAN_PRODUK WHERE PERIKSA_ID='$idperiksa[0]') AS JUMLAH_TEMUAN, A.SARANA_ID, A.NAMA_SARANA, A.ALAMAT_1, A.ALAMAT_2, A.JENIS_INDUSTRI, A.NAMA_PIMPINAN, A.PENANGGUNG_JAWAB, A.KEGIATAN_SARANA, B.PERIKSA_ID, B.JENIS_SARANA_ID, B.BBPOM_ID, STUFF(dbo.GROUP_KLASIFIKASI(B.PERIKSA_ID),1,1,'') AS KK_ID, CONVERT(VARCHAR(10), B.AWAL_PERIKSA, 103) AS AWAL_PERIKSA, CONVERT(VARCHAR(10), B.AKHIR_PERIKSA, 103) AS AKHIR_PERIKSA, C.TUJUAN_PEMERIKSAAN, C.KEPATUHAN_CPKB, C.LATAR_BELAKANG, C.PERUBAHAN_BERMAKNA, C.RUANG_LINGKUP, C.AREA_INSPEKSI, C.DISTRIBUSI_PENGANGKUTAN, C.PERMOHONAN_PENDAFTARAN_PRODUK, C.ISU_SPESIFIK_LAINNYA, C.SITE_FILE_MASTER, C.LAIN_LAIN, C.SAMPEL_DIAMBIL, C.DISTRIBUSI_LAPORAN, C.LAMPIRAN, C.KESIMPULAN_OBSERVASI, C.TINDAKAN_OBSERVASI, C.DIP_A, C.DIP_B, C.DIP_C, C.DIP_D, C.KESIMPULAN_DIP, C.TINDAK_LANJUT_DIP,C.TEMUAN_KRITIKAL, C.TEMUAN_MAJOR, C.TEMUAN_MINOR, B.HASIL, C.DETIL_HASIL, C.KESIMPULAN_DETIL_TMK, C.REKOMENDASI, C.KESIMPULAN, C.TINDAK_LANJUT, C.TIME_LINE, C.STATUS_CAPA, C.CAPA_CLOSED, D.NAMA_PRODUK, D.KLASIFIKASI_PRODUK, D.NETTO, D.NOMOR_REGISTRASI, D.SATUAN, D.NO_BATCH, D.TANGGAL_EXPIRE, D.NAMA_PRODUK, D.KLASIFIKASI_PRODUK, D.NOMOR_REGISTRASI, D.NO_BATCH, D.NETTO, D.TANGGAL_EXPIRE, D.NAMA_PERUSAHAAN, D.ALAMAT_PERUSAHAAN, D.JENIS_PELANGGARAN, D.KATEGORI, D.JUMLAH_TEMUAN, D.HARGA_SATUAN, D.HARGA_TOTAL, D.TINDAKAN_PRODUK, D.KETERANGAN_SUMBER, STUFF(dbo.GROUP_CONCAT(B.PERIKSA_ID),1,1,'') AS SURAT_ID FROM M_SARANA A LEFT JOIN T_PEMERIKSAAN B ON A.SARANA_ID = B.SARANA_ID LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPKB C ON B.PERIKSA_ID = C.PERIKSA_ID LEFT JOIN T_PEMERIKSAAN_TEMUAN_PRODUK D ON B.PERIKSA_ID = D.PERIKSA_ID WHERE B.SARANA_ID = '$sarana' AND B.PERIKSA_ID = '$idperiksa[0]'";
			$data = $sipt->main->get_result($query);
			...
			if($data){
				foreach($query->result_array() as $row){
					...
					$arrdata = array('sess' => $row,
									 ...
									 'temuan' => $sipt->main->get_observasi($idperiksa[0], "KOS", ""),
									 'temuan_produk' => $temuan_produk);
				}
				$arrdata['sel_hasil'] = $row['HASIL'];
				$arrdata['sel_tmk'] = explode("#", $row['DETIL_HASIL']);
		...
		$msword = new PHPDocx();
		$msword->addParagraph('FORM - LAPORAN INSPEKSI CPKB', array('text-align' => 'justify','font-weight' => 'bold'));
		$msword->addParagraph('');
		...
		$msword->addParagraph('');
		$msword->addParagraph('Kepatuhan CPKB dan Keputusan', array('text-align' => 'justify', 'font-weight' => 'bold'));
		$msword->addParagraph(preg_replace('/[^(\x20-\x7F)]*/', "",html_entity_decode($row['KEPATUHAN_CPKB'])), array('text-align' => 'justify'));
		...
		if($jmltemuan > 1){	
			...
			for($t=0; $t<$jmltemuan; $t++){
				...
				$msword->addParagraph(preg_replace('/[^(\x20-\x7F)]*/', "",$temuan[$t]['TEMUAN_TEKS']), array('text-align' => 'justify'));
		...
		$msword->addParagraph($row['KESIMPULAN_OBSERVASI'], array('text-align' => 'justify'));
		...
		$msword->addParagraph('Pemeriksaan DIP', array('text-align' => 'justify', 'font-weight' => 'bold'));
		$msword->addParagraph('Bagian I : Dokumen Administrasi & Ringkasan Produk', array('text-align' => 'justify', 'font-weight' => 'bold'));
		$msword->addParagraph(trim($row['DIP_A']) != "" ?  preg_replace('/[^(\x20-\x7F)]*/', "",html_entity_decode($row['DIP_A'])): "-", array('text-align' => 'justify'));
		$msword->addParagraph('Bagian II : Data Mutu Keamanan dan Bahan Kosmetik', array('text-align' => 'justify', 'font-weight' => 'bold'));
		$msword->addParagraph(trim($row['DIP_B']) != "" ?  preg_replace('/[^(\x20-\x7F)]*/', "",html_entity_decode($row['DIP_B'])): "-", array('text-align' => 'justify'));
		$msword->addParagraph('Bagian III : Data Mutu Kosmetik', array('text-align' => 'justify', 'font-weight' => 'bold'));
		$msword->addParagraph(trim($row['DIP_C']) != "" ?  preg_replace('/[^(\x20-\x7F)]*/', "",html_entity_decode($row['DIP_C'])): "-", array('text-align' => 'justify'));
		$msword->addParagraph('Bagian IV : Data Keamanan dan Kemanfaatan Kosmetik', array('text-align' => 'justify', 'font-weight' => 'bold'));
		$msword->addParagraph(trim($row['DIP_D']) != "" ?  preg_replace('/[^(\x20-\x7F)]*/', "",html_entity_decode($row['DIP_D'])): "-", array('text-align' => 'justify'));
		$msword->addParagraph('Kesimpulan DIP', array('text-align' => 'justify', 'font-weight' => 'bold'));
		$msword->addParagraph('Kesimpulan', array('text-align' => 'justify', 'font-weight' => 'bold'));
		$msword->addParagraph(trim($row['KESIMPULAN_DIP']) != "" ?  preg_replace('/[^(\x20-\x7F)]*/', "",html_entity_decode($row['KESIMPULAN_DIP'])): "-", array('text-align' => 'justify'));
		$msword->addParagraph('Tindak Lanjut', array('text-align' => 'justify', 'font-weight' => 'bold'));
		$msword->addParagraph(trim($row['TINDAK_LANJUT_DIP']) != "" ?  preg_replace('/[^(\x20-\x7F)]*/', "",html_entity_decode($row['TINDAK_LANJUT_DIP'])): "-", array('text-align' => 'justify'));
		...
		if($row['HASIL'] == "TMK"){
			...
			$msword->addParagraph('<table width="100%"><tr><td colspan="3"><b>Tindak Lanjut</b></td></tr><tr><td width="220">Tindak Lanjut Hasil Inspeksi</td><td width="10">&nbsp;</td><td>'.preg_replace('/[^(\x20-\x7F)]*/','',html_entity_decode($row['TINDAK_LANJUT'])).'</td></tr><tr><td width="220">Time Line</td><td width="10">&nbsp;</td><td>'.preg_replace('/[^(\x20-\x7F)]*/','',html_entity_decode($row['TIME_LINE'])).'</td></tr><tr><td colspan="3"><b>C A P A</b></td></tr><tr><td width="220">Hasil Evaluasi CAPA</td><td width="10">&nbsp;</td><td>'.preg_replace('/[^(\x20-\x7F)]*/','',html_entity_decode($row['STATUS_CAPA'])).'</td></tr></table>', array('text-align' => 'justify', 'font-weight' => 'bold'));
		}
		...
		$msword->output('INSPEKSI_CPKB_'.date('Ymd').'.doc');
	}
	...
	function prev_surat_tl($periksa){
		if($this->newsession->userdata('LOGGED_IN') == TRUE){
			...
			if($data){
				...
				$arrdata['major'] = $this->kritikal_major($periksa[0],'Major');
				$arrdata['kritikal'] = $this->kritikal_major($periksa[0],'Kritikal');
			}else{
				$arrdata = array('');
			}
			if($ispdf == FALSE){
				$ret = $this->load->view('pemeriksaan/tl/kosmetik/01KO/'.$row['PERIHAL'], $arrdata, true);
			}else{
				...
				$html = $this->load->view('pemeriksaan/tl/kosmetik/01KO/'.$row['PERIHAL'], $arrdata, true);
	...
	function kritikal_major($id, $params){
		$sipt =& get_instance();
		$this->load->model("main", "main", true);
		$query = "SELECT B.URAIAN, A.TEMUAN_TEKS FROM T_PEMERIKSAAN_PRODUKSI_CPKB_TEMUAN A LEFT JOIN M_TABEL B ON A.TEMUAN_OBSERVASI = B.KODE WHERE A.PERIKSA_ID = '$id' AND A.TEMUAN_KRITERIA = '$params' AND B.JENIS = 'TEMUAN_OBSERVASI_CPKP'";
		$data = $sipt->main->get_result($query);
		if($data){
			foreach($query->result_array() as $row){
				$result[] = $row;
