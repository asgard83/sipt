...
class F02LL extends Model{

	function GetForm02LL($sarana, $jenis, $klasifikasi, $idperiksa){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			...
			if($idperiksa==""){#Input Mode				  
				...
				$qsarana = "SELECT SARANA_ID, NAMA_SARANA, ALAMAT_1, ALAMAT_2, TELEPON, NOMOR_IZIN, TANGGAL_IZIN, NAMA_PIMPINAN, PENANGGUNG_JAWAB, NO_SIK FROM M_SARANA WHERE SARANA_ID='$sarana'";
				...
				if($dt_sarana){
					foreach($qsarana->result_array() as $row){
						$arrdata = array('sess' => $row,
										 ...
										 'temuan_produk' => '',
										 'sel_tujuan' => 'Rutin',
										 'act' => site_url().'/post/pemeriksaan/set_periksa/02LL/simpan',
				...
				*/
				...
				if($this->newsession->userdata('SESS_BBPOM_ID') != "92"){#Update dan Perbaikan Balai
					if($status=="20101"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/02LL/update';
					}else if($status=="20102" || $status =="20103"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/02LL/perbaikan';
					}else if($status =="60020"){
						  $arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/02LL/rekomendasi';
					}
			...
			$arrdata['cb_tindakan'] = $sipt->main->referensi("TL_DISTRIBUSI","",TRUE,TRUE);
	...
	function SaveForm02LL($action, $isajax){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			$sipt =& get_instance();
		...
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE ){
			...
			*/
			...
			if(array_key_exists('2', $this->newsession->userdata('SESS_KODE_ROLE'))){#Operator
				  ...
				  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02LL/operator';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02LL/spv-satu';
				...
				$arrdata['obj_status'] = 'SUPERVISOR[STATUS]';
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02LL/spv-dua';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/02LL/56';
			}
			if($subid!=""){
				$isPerbaikan = FALSE;
				$isEditTLBalai = FALSE;
	...
	function get_reupload($periksa, $jenis){
		...
		if($data){
			...
			$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/02LL/reupload-mapping';
		}
		return $arrdata;
	}
