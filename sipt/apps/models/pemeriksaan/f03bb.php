...
class F03BB extends Model{
	
	function GetForm03BB($sarana, $jenis, $klasifikasi, $idperiksa){
...
$qsarana = "SELECT NAMA_SARANA, ALAMAT_1, TELEPON, NOMOR_IZIN, TANGGAL_IZIN, NO_SIK, NAMA_PIMPINAN, PENANGGUNG_JAWAB FROM M_SARANA WHERE SARANA_ID='$sarana'";
				...
				if($dt_sarana){
					foreach($qsarana->result_array() as $row){
						$arrdata = array('sess' => $row,
										 ...
										 'temuan_produk' => '',
										 'sel_tujuan' => 'Rutin',
										 'act' => site_url().'/post/pemeriksaan/set_periksa/03BB/simpan',
				...
				*/
				...
				if($this->newsession->userdata('SESS_BBPOM_ID') != "92"){#Update dan Perbaikan Balai
					if($status=="20101"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/03BB/update';
					}else if($status=="20102" || $status =="20103"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/03BB/perbaikan';
					}else if($status=="60020"){
						$arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/03BB/rekomendasi';
					}
	...
	function SaveForm03BB($action, $isajax){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			$sipt =& get_instance();
		...
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE ){
			...
			*/
			...
			if(array_key_exists('2', $this->newsession->userdata('SESS_KODE_ROLE'))){#Operator
				  ...
				  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03BB/operator';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03BB/spv-satu';
				...
				$arrdata['obj_status'] = 'SUPERVISOR[STATUS]';
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03BB/spv-dua';
				...
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/03BB/56';
			}
			if($subid!=""){
				$isPerbaikan = FALSE;
				$isEditTLBalai = FALSE;
