...
class F01PK extends Model{
	
	function GetForm01PK($sarana, $jenis, $klasifikasi, $idperiksa){
...
$qsarana = "SELECT(SELECT COUNT(SARANA_ID) FROM T_PEMERIKSAAN WHERE SARANA_ID='$sarana' AND STATUS NOT IN('20')) AS PERIKSA_SEBELUMNYA, A.SARANA_ID, A.NAMA_SARANA, A.ALAMAT_1, A.ALAMAT_2, A.JENIS_INDUSTRI, A.NAMA_PIMPINAN, A.PENANGGUNG_JAWAB, A.KEGIATAN_SARANA FROM M_SARANA A LEFT JOIN T_PEMERIKSAAN B ON A.SARANA_ID = B.SARANA_ID WHERE A.SARANA_ID ='$sarana'";
				  ...
				  if($dt_sarana){
					  foreach($qsarana->result_array() as $row){
						  $arrdata = array('sess' => $row,
										   ...
										   'temuan_produk' => array(),
										   'temuan_cpotb' => array(),
										   'act' => site_url().'/post/pemeriksaan/set_periksa/01PK/simpan',
				...
				$status = $idperiksa[1];
				$qperiksa = "SELECT(SELECT COUNT(SARANA_ID) FROM T_PEMERIKSAAN WHERE SARANA_ID = '$sarana' AND STATUS IN('20')) AS PERIKSA_SEBELUMNYA, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_TEMUAN_PRODUK WHERE PERIKSA_ID='$idperiksa[0]') AS JUMLAH_TEMUAN, A.SARANA_ID, A.NAMA_SARANA, A.ALAMAT_1, A.ALAMAT_2, A.JENIS_INDUSTRI, A.NAMA_PIMPINAN, A.PENANGGUNG_JAWAB, A.KEGIATAN_SARANA, B.PERIKSA_ID, B.JENIS_SARANA_ID, B.BBPOM_ID, STUFF(dbo.GROUP_KLASIFIKASI(B.PERIKSA_ID),1,1,'') AS KK_ID, CONVERT(VARCHAR(10), B.AWAL_PERIKSA, 103) AS AWAL_PERIKSA, CONVERT(VARCHAR(10), B.AKHIR_PERIKSA, 103) AS AKHIR_PERIKSA, C.TUJUAN_PEMERIKSAAN, C.STANDARD, C.KEPATUHAN_CPPKB, C.LATAR_BELAKANG, C.PERUBAHAN_BERMAKNA, C.RUANG_LINGKUP, C.AREA_INSPEKSI, C.DISTRIBUSI_PENGANGKUTAN, C.PERMOHONAN_PENDAFTARAN_PRODUK, C.ISU_SPESIFIK_LAINNYA, C.SITE_FILE_MASTER, C.LAIN_LAIN, C.SAMPEL_DIAMBIL, C.DISTRIBUSI_LAPORAN, C.LAMPIRAN, C.TEMUAN_KRITIKAL, C.TEMUAN_MAJOR, C.TEMUAN_MINOR, B.HASIL, C.DETIL_HASIL, C.KESIMPULAN_DETIL_TMK, C.REKOMENDASI, C.KESIMPULAN, C.TINDAK_LANJUT, C.TIME_LINE, C.STATUS_CAPA, C.CAPA_CLOSED, D.NAMA_PRODUK, D.KLASIFIKASI_PRODUK, D.NETTO, D.NOMOR_REGISTRASI, D.SATUAN, D.NO_BATCH, D.TANGGAL_EXPIRE, D.NAMA_PERUSAHAAN, D.ALAMAT_PERUSAHAAN, D.KATEGORI, D.JUMLAH_TEMUAN, D.HARGA_SATUAN, D.HARGA_TOTAL, D.TINDAKAN_PRODUK, D.KETERANGAN_SUMBER, D.JENIS_PELANGGARAN, D.TINDAKAN_PRODUK, STUFF(dbo.GROUP_CONCAT(B.PERIKSA_ID),1,1,'') AS SURAT_ID FROM M_SARANA A LEFT JOIN T_PEMERIKSAAN B ON A.SARANA_ID = B.SARANA_ID LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPPKB C ON B.PERIKSA_ID = C.PERIKSA_ID LEFT JOIN T_PEMERIKSAAN_TEMUAN_PRODUK D ON B.PERIKSA_ID = D.PERIKSA_ID WHERE B.SARANA_ID = '$sarana' AND B.PERIKSA_ID = '$idperiksa[0]'";
				...
				if($dt_periksa){
				  ...
				  if($this->newsession->userdata('SESS_BBPOM_ID') != "94"){#Update dan Perbaikan Balai
					  if($status=="20101"){
						  $arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/01PK/update';
					  }else if($status=="20102" || $status =="20103"){
						  $arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/01PK/perbaikan';
					  }else if($status=="60020"){
						  $arrdata['act'] = site_url().'/post/pemeriksaan/set_periksa/01PK/rekomendasi';
					  }
				  ...
				  $arrdata['temuan_cpotb'] = $sipt->main->get_observasi($idperiksa[0], "PK", "");
			...
			$arrdata['tindakan_observasi'] = $sipt->main->referensi("TL_OBSERVASI","'05','06','07'",FALSE);
	...
	function get_temuan($sarana, $periksa, $isprev){
		...
		if($periksa==""){
			...
			$query = "SELECT (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_PRODUKSI_CPPKB_TEMUAN WHERE PERIKSA_ID='$periksa') AS JUMLAH_TEMUAN, A.SARANA_ID, C.SERI, C.TEMUAN_OBSERVASI, C.TEMUAN_TEKS, C.TEMUAN_KRITERIA, C.TEMUAN_FILE, D.URAIAN AS UR_JENIS FROM T_PEMERIKSAAN A LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPPKB B ON A.PERIKSA_ID = B.PERIKSA_ID LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPPKB_TEMUAN C ON B.PERIKSA_ID = C.PERIKSA_ID LEFT JOIN M_TABEL D ON D.KODE = C.TEMUAN_OBSERVASI WHERE A.PERIKSA_ID = '$periksa' AND A.SARANA_ID = '$sarana' AND D.JENIS='TEMUAN_OBSERVASI_CPOTB'";
	...
	function SaveForm01PK($action, $isajax){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE){
			$sipt =& get_instance();
			$this->load->model("main","main", true);
			...
			if($action=="simpan"){#Insert
				...
				if($this->db->insert('T_PEMERIKSAAN', $arr_pemeriksaan)){
					...
					foreach($this->input->post('PEMERIKSAAN_CPPKB') as $c => $d){
						if(!is_array($d))
							$arr_cpotb[$c] = $d;
						else
							$arr_cpotb[$c] = join("#", $d);
					...
					$this->db->insert('T_PEMERIKSAAN_PRODUKSI_CPPKB', $arr_cpotb);
					...
					if($this->input->post('TEMUAN')){
						...
						for($i=0;$i<count($arrtemuan[$arrkeys[0]]);$i++){
							$seri = (int)$sipt->main->get_uraian("SELECT MAX(SERI) AS MAXID FROM T_PEMERIKSAAN_PRODUKSI_CPPKB_TEMUAN WHERE PERIKSA_ID = '$periksa_id'", "MAXID") + 1;
							...
							$this->db->insert('T_PEMERIKSAAN_PRODUKSI_CPPKB_TEMUAN', $temuan);
						}
					}
				...
				if($this->db->update('T_PEMERIKSAAN', $arr_pemeriksaan)){
					...
					$arr_cpotb['KESIMPULAN_DETIL_TMK'] = htmlspecialchars_decode($arr_cpotb['KESIMPULAN_DETIL_TMK']);
					$this->db->where(array("PERIKSA_ID" => $this->input->post('PERIKSA_ID')));
					$this->db->update('T_PEMERIKSAAN_PRODUKSI_CPPKB', $arr_cpotb);
				}
				
				if($this->input->post('TEMUAN')){
					  $this->db->where('PERIKSA_ID', $this->input->post('PERIKSA_ID'));
					  $this->db->delete('T_PEMERIKSAAN_PRODUKSI_CPPKB_TEMUAN');
					  $arrtemuan = $this->input->post('TEMUAN');
					  $arrkeys = array_keys($arrtemuan);
					  for($i=0;$i<count($arrtemuan[$arrkeys[0]]);$i++){
						  $seri = (int)$sipt->main->get_uraian("SELECT MAX(SERI) AS MAXID FROM T_PEMERIKSAAN_PRODUKSI_CPPKB_TEMUAN WHERE PERIKSA_ID = '".$this->input->post('PERIKSA_ID')."'", "MAXID") + 1;
	...
	function get_preview($sarana,$id,$jenis){
		...
		$this->load->model("main","main", true);
		$id = explode(".", $id);
		$query = "SELECT (CAST(A.SARANA_ID AS VARCHAR) + '/' + A.JENIS_SARANA_ID + '/' + STUFF(dbo.GROUP_KLASIFIKASI(A.PERIKSA_ID),1,1,'') + '/' + CAST(A.PERIKSA_ID AS VARCHAR) + '.' + A.STATUS) AS IDPERIKSA, A.PERIKSA_ID, A.SARANA_ID, A.STATUS, CONVERT(VARCHAR(10), A.AWAL_PERIKSA, 103) AS [AWAL_PERIKSA], CONVERT(VARCHAR(10), A.AKHIR_PERIKSA, 103) AS [AKHIR_PERIKSA], B.KEPATUHAN_CPPKB, B.LATAR_BELAKANG, B.PERUBAHAN_BERMAKNA, B.RUANG_LINGKUP, B.AREA_INSPEKSI, B.TEMUAN_KRITIKAL, B.TEMUAN_MAJOR, B.TEMUAN_MINOR, A.HASIL, B.DETIL_HASIL, B.KESIMPULAN_DETIL_TMK, B.REKOMENDASI, B.KESIMPULAN, STUFF(dbo.GROUP_CONCAT(A.PERIKSA_ID),1,1,'') AS SURAT_ID FROM T_PEMERIKSAAN A LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPPKB B ON A.PERIKSA_ID = B.PERIKSA_ID WHERE A.SARANA_ID = '$sarana' AND A.PERIKSA_ID = '$id[0]'";
		
		$judul = $sipt->main->get_judul($jenis);
		$data = $sipt->main->get_result($query);
		$observasi = $sipt->main->get_observasi($id[0], "PK", "");
		if($data){
			foreach($query->result_array() as $row){
	...
	function input_preview($sarana,$jenis,$idperiksa,$subid){
		if($this->newsession->userdata('LOGGED_IN') ==  TRUE ){
			...
			$idperiksa = explode(".", $idperiksa);
			$stat = $idperiksa[1];
			$query = "SELECT(SELECT COUNT(SARANA_ID) FROM T_PEMERIKSAAN WHERE SARANA_ID = '$sarana' AND STATUS IN('20')) AS PERIKSA_SEBELUMNYA, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_TEMUAN_PRODUK WHERE PERIKSA_ID='$idperiksa[0]') AS JUMLAH_TEMUAN, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_PRODUKSI_CPOB_PERBAIKAN WHERE PERIKSA_ID='$idperiksa[0]') AS JML_PERBAIKAN, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_PROSES WHERE PERIKSA_ID = '$idperiksa[0]') AS JML_PROSES, (SELECT COUNT(PERIKSA_ID) FROM T_PEMERIKSAAN_PRODUKSI_CPPKB_PERBAIKAN WHERE PERIKSA_ID='$idperiksa[0]') AS JML_PERBAIKAN, A.SARANA_ID, A.NAMA_SARANA, A.ALAMAT_1, A.ALAMAT_2, A.JENIS_INDUSTRI, A.NAMA_PIMPINAN, A.PENANGGUNG_JAWAB, A.KEGIATAN_SARANA, B.PERIKSA_ID, B.JENIS_SARANA_ID, B.BBPOM_ID, STUFF(dbo.GROUP_KLASIFIKASI(B.PERIKSA_ID),1,1,'') AS KK_ID, CONVERT(VARCHAR(10), B.AWAL_PERIKSA, 103) AS AWAL_PERIKSA, CONVERT(VARCHAR(10), B.AKHIR_PERIKSA, 103) AS AKHIR_PERIKSA, C.TUJUAN_PEMERIKSAAN, C.KEPATUHAN_CPPKB, C.LATAR_BELAKANG, C.PERUBAHAN_BERMAKNA, C.RUANG_LINGKUP, C.AREA_INSPEKSI, C.DISTRIBUSI_PENGANGKUTAN, C.PERMOHONAN_PENDAFTARAN_PRODUK, C.ISU_SPESIFIK_LAINNYA, C.SITE_FILE_MASTER, C.LAIN_LAIN, C.SAMPEL_DIAMBIL, C.DISTRIBUSI_LAPORAN, C.LAMPIRAN, C.TEMUAN_KRITIKAL, C.TEMUAN_MAJOR, C.TEMUAN_MINOR, B.HASIL, C.DETIL_HASIL, C.KESIMPULAN_DETIL_TMK, C.REKOMENDASI, C.KESIMPULAN, C.TINDAK_LANJUT, C.HASIL_PUSAT, C.CATATAN_PUSAT, C.TIME_LINE, C.STATUS_CAPA, C.CAPA_CLOSED, D.NAMA_PRODUK, D.KLASIFIKASI_PRODUK, D.NETTO, D.NOMOR_REGISTRASI, D.SATUAN, D.NO_BATCH, D.TANGGAL_EXPIRE, D.NAMA_PERUSAHAAN, D.ALAMAT_PERUSAHAAN, D.KATEGORI, D.JUMLAH_TEMUAN, D.HARGA_SATUAN, D.HARGA_TOTAL, D.TINDAKAN_PRODUK, D.KETERANGAN_SUMBER, D.JENIS_PELANGGARAN, D.TINDAKAN_PRODUK, STUFF(dbo.GROUP_CONCAT(B.PERIKSA_ID),1,1,'') AS SURAT_ID FROM M_SARANA A LEFT JOIN T_PEMERIKSAAN B ON A.SARANA_ID = B.SARANA_ID LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPPKB C ON B.PERIKSA_ID = C.PERIKSA_ID LEFT JOIN T_PEMERIKSAAN_TEMUAN_PRODUK D ON B.PERIKSA_ID = D.PERIKSA_ID WHERE B.SARANA_ID = '$sarana' AND B.PERIKSA_ID = '$idperiksa[0]'";
			$data = $sipt->main->get_result($query);
			if($data){
				foreach($query->result_array() as $row){
					$temuan_produk[] = $row;
			...
			if(array_key_exists('2', $this->newsession->userdata('SESS_KODE_ROLE'))){#Operator
				if($this->newsession->userdata('SESS_BBPOM_ID') != "94"){
					$isEditTL = TRUE;
					$arrdata['obj_ot'] = 'PEMERIKSAAN_CPPKB';
				...
				$isEditTLBalai = TRUE;
				$arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/01PK/operator';
			  ...
			  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/01PK/spv-satu';
			  ...
			  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/01PK/spv-dua';
			  $arrdata['obj_status'] = 'SUPERVISOR[STATUS]';
			  ...
			  $arrdata['act'] = site_url().'/post/pemeriksaan/set_proses/01PK/56';
		  }
		...
		if($this->newsession->userdata('LOGGED_IN') == TRUE){
			...
			if($role=="operator"){
				...
				if($this->db->update('T_PEMERIKSAAN', $arr_status)){#Update Status Operator
					  if($this->input->post('PEMERIKSAAN_CPPKB')){
						  foreach($this->input->post('PEMERIKSAAN_CPPKB') as $c => $d){
							  if(!is_array($d))
								$arr_update[$c] = $d;
							  else
						  ...
						  $this->db->where(array("PERIKSA_ID" => $this->input->post('PERIKSA_ID')));
						  $this->db->update('T_PEMERIKSAAN_PRODUKSI_CPPKB', $arr_update); #Update Tindak Lanjut Pusat
					  }
					  ...
					  if($this->input->post('PERBAIKAN')){
						  $perbaikan = (int)$sipt->main->get_uraian("SELECT MAX(SERI) AS MAXID FROM T_PEMERIKSAAN_PRODUKSI_CPPKB_PERBAIKAN WHERE PERIKSA_ID = '".$this->input->post('PERIKSA_ID')."'", "MAXID") + 1;
						  ...
						  if(trim($arr_perbaikan['TANGGAL_PERBAIKAN']) != "" || trim($arr_perbaikan['DETAIL_PERBAIKAN']) != ""){
							  $this->db->insert('T_PEMERIKSAAN_PRODUKSI_CPPKB_PERBAIKAN', $arr_perbaikan);#Perbaikan Pemeriksaan					  
						  }
					  }
				...
				if($this->db->update('T_PEMERIKSAAN', $arr_status)){
					  $arr_input = $this->input->post('PEMERIKSAAN_CPPKB'); 
					  ...
					  $this->db->where(array("PERIKSA_ID" => $this->input->post('PERIKSA_ID')));
					  $this->db->update('T_PEMERIKSAAN_PRODUKSI_CPPKB', $arr_update); #Update Tindak Lanjut
	...
	function set_bap($sarana,$jenis,$idperiksa,$subid){
			...
			if($data){
				foreach($query->result_array() as $row){
					...
					$arrdata = array('sess' => $row,
									 ...
									 'temuan' => $sipt->main->get_observasi($idperiksa[0], "PK", ""));
				}
				$arrdata['sel_hasil'] = $row['HASIL'];
				$arrdata['sel_tmk'] = explode("#", $row['DETIL_HASIL']);
				$arrdata['detil_kesimpulan_tmk'] = explode("#", $row['KESIMPULAN_DETIL_TMK']);
		...
		$msword = new PHPDocx();
		$msword->addParagraph('FORM - LAPORAN INSPEKSI', array('text-align' => 'justify','font-weight' => 'bold'));
		$msword->addParagraph('');
		...
		$msword->addParagraph(preg_replace('/[^(\x20-\x7F)]*/', "",html_entity_decode($row['KEPATUHAN_CPPKB'])), array('text-align' => 'justify'));
...
$msword->addParagraph('Distribusi Laporan', array('text-align' => 'justify', 'font-weight' => 'bold'));
	...
	function get_perbaikan($periksa_id){
		$query = "SELECT A.SARANA_ID, B.SERI AS NO, CONVERT(VARCHAR(10), B.TANGGAL_PERBAIKAN, 103) AS [TANGGAL PERBAIKAN], B.DETAIL_PERBAIKAN AS [DETAIL PERBAIKAN], '<a class=\"normal\" href=\"".base_url()."files/' + CAST(A.SARANA_ID AS VARCHAR)+ '/'+ B.FILE_PERBAIKAN +'\" target=\"_blank\">View</a>' AS [FILE PERBAIKAN], C.NAMA_BBPOM AS [BALAI ATAU UNIT], D.NAMA_USER AS [NAMA PETUGAS] FROM T_PEMERIKSAAN A LEFT JOIN T_PEMERIKSAAN_PRODUKSI_CPPKB_PERBAIKAN B ON A.PERIKSA_ID = B.PERIKSA_ID LEFT JOIN T_USER D ON B.CREATE_BY = D.USER_ID LEFT JOIN M_BBPOM C ON D.BBPOM_ID = C.BBPOM_ID WHERE B.PERIKSA_ID='$periksa_id'";
		$this->load->library('newtable');
		$this->newtable->search(array(array('', '')));
